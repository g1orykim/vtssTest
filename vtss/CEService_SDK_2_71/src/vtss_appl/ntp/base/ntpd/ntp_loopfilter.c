...
#else
int	allow_panic = TRUE;	    /* allow panic correction */
int	mode_ntpdate = FALSE;	/* exit on first clock set */
#endif
...
int	ntp_state;	    /* clock discipline state */
#endif
	...
	)
{
	...
	if (debug)
		printf(
		    ...
		    peer->associd, fp_offset, drift_comp * 1e6, ntp_state);
#endif
#ifdef LOCKCLOCK
	return (0);
...
#ifdef DEBUG
    if (debug) {   		
	    printf("\n***local_clock**** fabs(fp_offset):%.6f, fp_offsset:%.6f, clock_max:%.6f\n", 
	        fabs(fp_offset), fp_offset, clock_max);
	}
#endif

#ifdef NTP_ECOS	
	if (fabs(fp_offset) > clock_max && clock_max > 0) {        
        vtss_ntp_last_sys_info(0, fp_offset, ntp_state);
    } else {
        vtss_ntp_last_sys_info(1, fp_offset, ntp_state);       
    }    
#endif
...
#ifdef DEBUG
            if (debug) {   		
	                printf("\nSPIKE mu (%u)< clock_minstep(%f) return xxx\n", mu, clock_minstep);
	        }
#endif			    
			if (mu < clock_minstep)
				return (0);
...
#ifdef NTP_ECOS
            ntp_step_count++;
            vtss_ntp_step_count_set(ntp_step_count);
#endif
            step_systime(fp_offset);
...
#ifdef DEBUG
        if (debug) {   		
	        printf("\nxxx The offset is less than the step threshold xxx\n");
	    }
#endif		
...
#ifdef DEBUG
                if (debug) {   	
	                printf("\nxxx mu (%u)< clock_minstep(%f) return xxx\n", mu, clock_minstep);
	            }
#endif			    
				return (0);
            }
...
#ifdef NTP_ECOS
			vtss_ntp_freq_info(mu,fp_offset, clock_offset, clock_frequency);
#endif   
			rstclock(S_SYNC, peer->epoch, fp_offset);
			break;
...
#ifdef DEBUG
	if (debug)
		printf("clock_frequency %.0f \n",clock_frequency * 1e6);
#endif /* DEBUG */		
		clock_frequency = drift_comp + clock_frequency +
		    flladj + plladj;
#ifdef DEBUG
	if (debug) {
		printf("clock_frequency %.0f PPM, drift_comp %.0f PPM, flladj %.0f PPM plladj %.0f PPM\n",clock_frequency * 1e6, drift_comp* 1e6 , flladj* 1e6, plladj* 1e6);
    }		
#endif /* DEBUG */		    
...
#ifdef DEBUG
	if (debug)
		printf("frequency error %.0f PPM \n",clock_frequency * 1e6);
		printf("frequency tolerance %.0f PPM \n",NTP_MAXFREQ * 1e6);
#endif /* DEBUG */    		    
    ...
    vtss_ntp_adj_time(drift_comp, sys_clocktime, fp_offset, ntoa(&peer->srcadr));
	return (rval);
#endif /* LOCKCLOCK */
}
...
#ifdef DEBUG
	if (debug)
		printf("1111  rstclock: time %lu offset %.6f freq %.3f state %d\n",
	...
	)
{
	...
	case LOOP_HUFFPUFF:		/* huff-n'-puff filter length */
        ...
        if ( sys_huffpuff == NULL ) {
            msyslog(LOG_ERR, "loop_config: memory insufficient\n");
            return;
        }
#endif
		for (i = 0; i < sys_hufflen; i++)
