		    ...
		    u_char *pdu_data, size_t pdu_data_len)
{
    ...
    /* Using malloc memory to avoid stack overflow */
    spdu_buf = malloc(PACKET_LENGTH);
    if ( !spdu_buf) {
        return -1;
    }
    /*
    ...
    if (sec_params == NULL) { free(spdu_buf); return -1;}
     ...
     */
    cp = NULL; *out_length = PACKET_LENGTH;
    result =
	   ...
	   int  *af_family)
{
	...
	case SNMP_MSG_TRAP:
	    ...
	    if (pdu->enterprise_length == SNMP_DEFAULT_ENTERPRISE_LENGTH){
	        ...
	        if (pdu->enterprise == NULL) { /* Sgetz, 04/2013, return when alloc memory fail. C³ */
	...
	if (pdu->community_len == 0){
	    ...
	    if (pdu->community == NULL) { /* Sgetz, 04/2013, return when alloc memory fail.C³ */
	        session->s_snmp_errno = SNMPERR_MALLOC;
	        return -1;
	    }
...
snmp_pdu_build (struct snmp_pdu *pdu, u_char *cp, size_t *out_length)
{
  ...
  size_t length, tmp_len = *out_length;
  int var_count = 0;
  u_char *tmp = NULL;
  ...
  for(vp = pdu->variables; vp; vp = vp->next_variable, var_count++){
    cp = snmp_build_var_op(cp, vp->name, &vp->name_length, vp->type,
			   vp->val_len, (u_char *)vp->val.string,
			   out_length);
    if (cp == NULL || ( pdu->version == SNMP_VERSION_3 && *out_length <= SNMP_MAX_MSG_V3_HDRS )  ) {
        break;
    }
    tmp = cp;
    tmp_len = *out_length;
	   ...
	   int *af_family)
{
	...
	case SNMPERR_USM_DECRYPTIONERROR:
		...
		if (0 == snmp_sess_send(sessp, pdu2)) { /* Sgetz, 04/2013, Fix the memory leakage issue.C³ */
	        snmp_free_pdu(pdu2);
	        /* TODO: indicate error */
	    }
	  }
...
snmp_pdu_parse(struct snmp_pdu *pdu, u_char  *data, size_t *length, int *af_family) {
  ...
  /* check all PUD type here and return -1 if PDU type is unknown */
  case  SNMP_MSG_GET:
  case  SNMP_MSG_GETNEXT:
  case  SNMP_MSG_SET:
  case  SNMP_MSG_GETBULK:
  ...
  default:
    ...
    /* 
        Issue:-
        -----
        DUT is not responding properly to an invalid PDU (type 0x42) and gives
        "genErr". This is caught by SilverCreek tool.

        The outcome from SilverCreek is :

        Details of Test 2.5.1.7
        2.5.1.x   The purpose of these tests is to verify that the agent handles
        malformed ASN.1 elements correctly and without catastrophic failure in the
        agent.

        2.5.1.7   This test sends an unsolicited response PDU, a v1 trap PDU
        (containing incorrect ASN.1 BER encoding in varbinds part) and an invalid PDU
        (type 0x42) to the agent.

        The test will issue a warning if the agent does not increment
        snmpInASNParseErrs (or snmpUnknownPDUHandlers if implemented in a multi-lingual
        agent) for the response, trap PDU and invalid PDU types.
    */
        return -1;
  }
	       ...
	       fd_set *fdset)
{
    ...
    if (sp->flags & SNMP_FLAGS_STREAM_SOCKET ) {
      ...
      if (isp->packet == NULL) { /* Sgetz, 04/2013, return when alloc memory fail.C³ */
              /* TODO FIX: recover ?? */
              isp->packet_size = 0;
              return -1;
      }
	...
	if ( pdu == NULL ) { /* Sgetz, 04/2013, return when alloc memory fail.C³ */
		return -1;
	}
      ...
      for(rp = isp->requests; rp; orp = rp, rp = rp->next_request) {
        ...
        if (pdu->version == SNMP_VERSION_3) {
		  ...
		  if (!snmpv3_verify_msg(rp,pdu)){
			  /* Sgetz, 04/2013, fix memory leakage issueC³ */
          ...
          if (pdu->command == SNMP_MSG_REPORT) {
            ...
            if (!sp->securityEngineIDLen && pdu->securityEngineIDLen) {
              ...
              if (sp->securityEngineID == NULL) {
                  /* Sgetz, 04/2013, fix memory leakage issue, no further processing */
                  snmp_free_pdu(rp->pdu);
                  free((char *)rp);
                  break;
              }
              ...
              if (!sp->contextEngineIDLen) {
                ...
                if (sp->contextEngineID == NULL) {
                    /* Sgetz, 04/2013, fix memory leakage issue, no further processing */
                    free(sp->securityEngineID);
                    sp->securityEngineIDLen = 0;
                    snmp_free_pdu(rp->pdu);
                    free((char *)rp);
                    break;
                }
		      ...
		      size_t len)
{
    ...
    switch(type){
      ...
      case ASN_PRIV_EXCL_RANGE:
		...
		if (vars->val.objid == NULL) { /* Sgetz, 04/2013, return when alloc memory fail.C³ */
			return NULL;
		}
      ...
      case ASN_NSAP:
        ...
        if (vars->val.string == NULL) { /* Sgetz, 04/2013, return when alloc memory fail.C³ */
            return NULL;
        }
