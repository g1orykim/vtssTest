	...
	)
{
	...
	if ((ret = (Queue)ntp_callout_malloc(sizeof(struct List))) == NULL)
	...
	)
{
	...
	rbuf.receiver = receive;
        ...
        if ((rbuf.dstadr = ntp_callout_malloc(sizeof(struct interface))) == NULL)
	...
	)
{
	 ...
	 * time.
	 */
	if ((rbuf = ntp_callout_malloc(sizeof(struct recvbuf))) == NULL)
