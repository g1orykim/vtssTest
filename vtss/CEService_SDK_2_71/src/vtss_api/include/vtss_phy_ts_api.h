...
typedef struct vtss_phy_ts_pps_config_s {
    ...
    BOOL  pps_output_enable; /**< PPS pulse output is enabled for this port */
...
typedef struct {
    struct {
#define VTSS_PHY_TS_IP_VER_4    0x00 /**< Version: IPv4 */
#define VTSS_PHY_TS_IP_VER_6    0x01 /**< Version: IPv6 */
...
typedef struct {
    struct {
        ...
        struct {
            u16   value;
            u16   mask;
        } proto_id; /**< PID: identifier of payload as defined in FRC 5718, only for PTP */
    } comm_opt; /**< ACH common config */

    /* No flow config reqd for ACH */
} vtss_phy_ts_ach_conf_t;
...
typedef struct {
    ...
    } domain; /**< PTP domain number config */
