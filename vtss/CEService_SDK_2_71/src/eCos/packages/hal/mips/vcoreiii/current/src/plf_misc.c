...
#if defined(CYG_HAL_MIPS_VCOREIII_DUAL_JAGUAR)
void vcoreiii_gpio_set_alternate_sec(int gpio, int is_alternate)
{
    cyg_uint32 mask = VTSS_BIT(gpio);
    if(is_alternate) {
        vcoreiii_io_set_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_ALT(0), mask);
        vcoreiii_io_clr_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_ALT(1), mask);
    } else {
        vcoreiii_io_clr_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_ALT(0), mask);
        vcoreiii_io_clr_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_ALT(1), mask);
    }
}

void vcoreiii_gpio_set_input_sec(int gpio, int is_input)
{
    vcoreiii_gpio_set_alternate_sec(gpio, 0);
    if(is_input)
        vcoreiii_io_clr_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_OE, VTSS_BIT(gpio));
    else
        vcoreiii_io_set_sec(&VTSS_DEVCPU_GCB_GPIO_GPIO_OE, VTSS_BIT(gpio));
}
#endif  /* CYG_HAL_MIPS_VCOREIII_DUAL_JAGUAR */
...
CYG_I2C_VCORE_DEVICE(i2c_si570_vcxo, 0x55);
...
static inline void hal_vcoreiii_reset_secondary(void)
{
#if defined(CYG_HAL_MIPS_VCOREIII_DUAL_JAGUAR)
    if (vcoreiii_has_secondary) {
        /* Reset secondary device */
        vcoreiii_io_sec_writel(VTSS_F_DEVCPU_GCB_DEVCPU_RST_REGS_SOFT_CHIP_RST_SOFT_CHIP_RST,
...
void hal_vcoreiii_reset(void)
{
    ...
    // Reset optional secondary device
    hal_vcoreiii_reset_secondary();
    // Reset the whole device, i.e. both CPU, SwC (and possibly PHYs).
    VTSS_DEVCPU_GCB_DEVCPU_RST_REGS_SOFT_CHIP_RST = VTSS_F_DEVCPU_GCB_DEVCPU_RST_REGS_SOFT_CHIP_RST_SOFT_CHIP_RST;
...
hal_map_iospace(void)
{
    ...
    create_tlb(0, VTSS_TO_DEVCPU_ORG, SZ_16M, MMU_REGIO_RW, MMU_REGIO_RW);
    
    /* 0x70000000 - 0x70ffffff */
    create_tlb(1, VTSS_TO_CFG,        SZ_16M, MMU_REGIO_RW, MMU_REGIO_RW);
    ...
    /* 0x50000000: PI CS0-CS4, 64M each */
    create_tlb(4, VTSS_MEMCTL2_TO,            SZ_64M, MMU_REGIO_RW, MMU_REGIO_RW);
    create_tlb(5, VTSS_MEMCTL2_TO + 2*SZ_64M, SZ_64M, MMU_REGIO_RW, MMU_REGIO_RW);
#else
    /* 0x50000000: PI, only 4 address pins (for NAND etc) */
    create_tlb(4, VTSS_MEMCTL2_TO, SZ_4K, MMU_REGIO_RW, MMU_REGIO_INVAL);
#endif
}
...
void hal_platform_init(void)
{
    ...
    /* Enable UART/GPIO - bit 31+30 alternate function '01' */
...
#else
#warning Unknown board flavour - UART may not work
#endif
...
#if defined(CYG_HAL_MIPS_VCOREIII_DUAL_JAGUAR)
    /* GPIO 18 + cs is used for Secondary switch */
    vcoreiii_gpio_set_alternate(18 + CYGNUM_HAL_MIPS_VCOREIII_DUAL_JAGUAR_SECONDARY_CS, 1);

    /* Configure Slow PI */
    val = VTSS_ICPU_CFG_PI_MST_PI_MST_CFG;
    val &= ~VTSS_M_ICPU_CFG_PI_MST_PI_MST_CFG_CLK_DIV; /* Clr divisor bitfield */
    val |= VTSS_F_ICPU_CFG_PI_MST_PI_MST_CFG_CLK_DIV(0x1F); /* Set value '1F' */
    VTSS_ICPU_CFG_PI_MST_PI_MST_CFG = val;
    
    /* Enable the PI interface */
    vcoreiii_io_set(VTSS_F_ICPU_CFG_CPU_SYSTEM_CTRL_GENERAL_CTRL_IF_MASTER_PI_ENA, &VTSS_ICPU_CFG_CPU_SYSTEM_CTRL_GENERAL_CTRL);

    /* Reset secondary device (if it is there) */
    vcoreiii_io_sec_writel(VTSS_F_DEVCPU_GCB_DEVCPU_RST_REGS_SOFT_CHIP_RST_SOFT_CHIP_RST,
                           &VTSS_DEVCPU_GCB_DEVCPU_RST_REGS_SOFT_CHIP_RST);
    ...
    /* Initialize secondary device for '16-bit ndone' mode (again) */
    vcoreiii_io_sec_writel(0x18181818, &VTSS_DEVCPU_PI_PI_PI_MODE);
    vcoreiii_io_sec_writel(0x18181818, &VTSS_DEVCPU_PI_PI_PI_MODE);

    /* Secondary CS PI settings */
    vcoreiii_io_writel(VTSS_BIT(17) | /* PI_nCS inactive between xfers */
                       /* 16bit */
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_DATA_WID | 
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_DEVICE_PACED_XFER_ENA |
                       /* Enable timeout */
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_DEVICE_PACED_TIMEOUT_ENA |
                       /* 1024 PI_Clk cycles */
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_DEVICE_PACED_TIMEOUT(6) |
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_WAITCC(1) |
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_CSCC(1) |
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_OECC(2) |
                       VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_HLDCC(3),
                       &VTSS_ICPU_CFG_PI_MST_PI_MST_CTRL(CYGNUM_HAL_MIPS_VCOREIII_DUAL_JAGUAR_SECONDARY_CS));

    if(VTSS_X_DEVCPU_GCB_CHIP_REGS_CHIP_ID_PART_ID(vcoreiii_io_sec_readl(&VTSS_DEVCPU_GCB_CHIP_REGS_CHIP_ID)) ==
       VTSS_X_DEVCPU_GCB_CHIP_REGS_CHIP_ID_PART_ID(VTSS_DEVCPU_GCB_CHIP_REGS_CHIP_ID)) {
        vcoreiii_has_secondary++;

        /* Disable timeouts, now we know its there */
        vcoreiii_io_clr(VTSS_F_ICPU_CFG_PI_MST_PI_MST_CTRL_DEVICE_PACED_TIMEOUT_ENA |
                        VTSS_M_ICPU_CFG_PI_MST_PI_MST_CTRL_DEVICE_PACED_TIMEOUT,
                        &VTSS_ICPU_CFG_PI_MST_PI_MST_CTRL(CYGNUM_HAL_MIPS_VCOREIII_DUAL_JAGUAR_SECONDARY_CS));

        hal_init_irq_secondary();
    } else {
    ...
    /* Bump up PI interface - as fast as possible = clk/4 = 50MHz */
    val = VTSS_ICPU_CFG_PI_MST_PI_MST_CFG;
    val &= ~VTSS_M_ICPU_CFG_PI_MST_PI_MST_CFG_CLK_DIV; /* Clr divisor bitfield */
    val |= VTSS_F_ICPU_CFG_PI_MST_PI_MST_CFG_CLK_DIV(4); /* clk/4 */
    VTSS_ICPU_CFG_PI_MST_PI_MST_CFG = val;
    /* individual devices on PI should set their waitcc if needed */
}
...
hal_clock_initialize(cyg_uint32 period)
{
    ...
    VTSS_ICPU_CFG_TIMERS_TIMER_TICK_DIV = CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER-1;
 ...
 * The timers are clocked with a 250MHz clock, and are prescaled by CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER.
 * By default, the prescaler is exactly one microsecond, so the compile time check below
 * will succeed causing a fast non-CPU-consuming computation of the timer value.
 *------------------------------------------------------------------------------*/
static inline cyg_uint32 timer_microseconds_to_timer_value(cyg_uint32 us)
{
    cyg_uint64 result;

    if (((CYGNUM_HAL_ARM_VCOREIII_TIMER_CLOCK) / (CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER)) == 1000000U) {
        result = us;
    } else {
        result = ((cyg_uint64)us * ((CYGNUM_HAL_ARM_VCOREIII_TIMER_CLOCK) / (CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER))) / 1000000ULL;
    }
    if (result > 0) {
        result--;
    }
...
static inline cyg_uint32 timer_value_to_microseconds(cyg_uint32 value)
{
    ...
    if (((CYGNUM_HAL_ARM_VCOREIII_TIMER_CLOCK) / (CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER)) != 1000000U) {
        ...
        result = (cyg_uint32)((result * 1000000ULL) / (cyg_uint64)(((CYGNUM_HAL_ARM_VCOREIII_TIMER_CLOCK) / (CYGNUM_HAL_ARM_VCOREIII_TIMER_DIVIDER))));
    }
    return (cyg_uint32)result;
}
...
cyg_uint64 hal_time_get(void)
{
    ...
    if (VTSS_ICPU_CFG_INTR_ICPU_IRQ0_IDENT & VTSS_F_ICPU_CFG_INTR_ICPU_IRQ0_IDENT_ICPU_IRQ0_TIMER0_IDENT) {
      result++;
      regval = VTSS_ICPU_CFG_TIMERS_TIMER_VALUE(0);
    }
