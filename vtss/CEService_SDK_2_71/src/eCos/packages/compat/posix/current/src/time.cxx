...
#if 0 /* Bug# 5748 - remove the clock_settime function */
...
externC int gettimeofday(struct timeval* tv, struct timezone* tz)
{
    ...
    cyg_uint64 t = Cyg_WallClock::wallclock->get_current_time_usecs();
...
// -------------------------------------------------------------------------
// settimeofday()
// Set the current time from a struct timeval
externC int settimeofday(const struct timeval* tv, const struct timezone* tz)
{
    Cyg_WallClock::wallclock->set_current_time_usecs(tv->tv_sec * 1000000ULL + tv->tv_usec);
    return 0;
}
