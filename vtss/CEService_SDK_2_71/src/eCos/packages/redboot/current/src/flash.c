...
fis_load(int argc, char *argv[])
{
    ...
    num_options = 2;
#if defined(CYGSEM_REDBOOT_FIS_CRC_CHECK) && defined(CYGPRI_REDBOOT_ZLIB_FLASH)
    init_opts(&opts[2], 'f', false, OPTION_ARG_TYPE_FLG, 
              (void *)&force_decompress, (bool *)0, "force decompress even if crc error");
    num_options++;
#endif  /* defined(CYGSEM_REDBOOT_FIS_CRC_CHECK) && defined(CYGPRI_REDBOOT_ZLIB_FLASH) */
#ifdef CYGPRI_REDBOOT_ZLIB_FLASH
    ...
    if (decompress
        ...
        bool try_decompress = true;
#ifdef CYGSEM_REDBOOT_FIS_CRC_CHECK
        cyg_flash_read(img->flash_base, (void *)mem_addr, img->data_length, &err_addr);
        ...
        if (cksum != img->file_cksum) {
            ...
            if(!force_decompress) {
                diag_printf("** Use -f to force decompress despite CRC error.\n");
                try_decompress = false;
            }
        }
#endif  /* CYGSEM_REDBOOT_FIS_CRC_CHECK */
    ...
    if(entry_address == (unsigned long)NO_MEMORY) {
        ...
        strcpy(fis_name_of_latest_loaded_image, name); // name is guaranteed to be within 16 bytes, since fis_lookup() would otherwise fail
    }
}
