...
# if !defined(VMS)
#  if defined(HAVE_TIMER_CREATE) && defined(HAVE_TIMER_SETTIME)
	if (timer_create (CLOCK_MONOTONIC, NULL, &ntpd_timerid) ==
#	ifdef SYS_VXWORKS
		ERROR
#	else
		-1
...
/*
 * reset_timer - initialize the timer data structures
 */
#ifdef NTP_ECOS
void
reset_timer(void)
{
    ...
    timer_t     old_ntpd_timerid;
#if 0 //James rest instead of init
	/*
	 * Initialize...
	 */
	alarm_flag = 0;
	...
	timer_xmtcalls = 0;
	timer_timereset = 0;
#endif //James rest instead of init
    old_ntpd_timerid = 	ntpd_timerid;   
#  if defined(HAVE_TIMER_CREATE) && defined(HAVE_TIMER_SETTIME)
	if (timer_create (CLOCK_MONOTONIC, NULL, &ntpd_timerid) ==
#	ifdef SYS_VXWORKS
		ERROR
...
#	endif
    ...
    if (timer_delete(old_ntpd_timerid) == -1)
    {
		fprintf (stderr, "timer create FAILED\n");
		exit (0);
	}
