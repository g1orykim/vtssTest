...
/** 
 * \brief
 * Control if frames are arborted if the IFH.ENCAP.WORD16_POP_CNT field is
 ...
 * 
 * Use the un-mapped port number, if the mapped value is outside port range
 * (default)
 ...
 * outgoing frame's outer tag contains a TPID for which PVE_TPID_AWARE_DIS
 * is set, then the port VOE sees the frame as untagged.
 * 
 ...
 * Bit0: TPID = 0x8100.
 * Bit1: TPID = 0x88A8
 * Bit2: TPID = REW::VLAN_TPID_CFG[0].TPID_VAL
 * Bit3: TPID = REW::VLAN_TPID_CFG[1].TPID_VAL
 * Bit4: TPID = REW::VLAN_TPID_CFG[2].TPID_VAL
 *
...
/** 
 * \brief
 * Defaul PCP value used by the OAM port VOE. This value is used for port
...
/** 
 * \brief
 * Defaul DEI value used by the OAM port VOE. This value is used for port
 * VOE counter updates when no outer Q-Tag is present in a frame.
 * 
 * The configuration  is used by the physical port and is not changed for
 * TX-Mirrored frames.
...
/** 
 * \brief
 * Enable lookup in the ES0 VCAP to control advanced frame modifications.

 *
 * \details 
 * 0: ES0 do not control frame modifications
 * 1: ES0 controls all frame rewrites
 *
 ...
 * 0: 0x8100
 * 1: 0x88A8
 * 2: Custom1. REW::TPID_CFG:TPID_VAL:0
 * 3: Custom2. REW::TPID_CFG:TPID_VAL:1
 * 4: Custom3. REW::TPID_CFG:TPID_VAL:2
 ...
 * 
 * Frame are send with the IFH preserved. The IFH is encapsulated according
 ...
 * No other rewrites are done to the frame.
 * 
 ...
 * 1: DEV10G::MAC_CFG_STATUS:MAC_PREAMBLE_CFG controls the placement.
 *
 ...
 * 1 : TC_VAL:N
 * 2-3: Reserved
 * 4: Mapped using mapping table 0, otherwise use LABEL_VAL:TC_VAL:N
 * 5: Mapped using mapping table 1, otherwise use mapping table 0
 * 6: Mapped using mapping table 2, otherwise use LABEL_VAL:TC_VAL:N
 ...
 * 2-3: Reserved
 * 4: Mapped using mapping table 0, otherwise use LL_TAG_VAL:TAG_PCP_VAL:n
 * 5: Mapped using mapping table 1, otherwise use mapping table 0
 * 6: Mapped using mapping table 2, otherwise use LL_TAG_VAL:TAG_PCP_VAL:n
 * 7: Mapped using mapping table 3, otherwise use mapping table 2
 *
 ...
 * 2: DP: REW:PORT:PORT_DP_MAP[DP]
 * 3: Reserved
 * 4: Mapped using mapping table 0, otherwise use LL_TAG_VAL:TAG_DEI_VAL:n
 * 5: Mapped using mapping table 1, otherwise use mapping table 0
 * 6: Mapped using mapping table 2, otherwise use LL_TAG_VAL:TAG_DEI_VAL:n
 * 7: Mapped using mapping table 3, otherwise use mapping table 2
 *
 ...
 *
 * \details 
 * 0: Encapsulation TAG_TPID_VAL:n
 * 1: Classified.  ANA controls via IFH:
 * If ifh.encap.tag_tipd = STD_TPID: 
 *   If ifh.vstax.tag_type = 0 then 0x8100 else TAG_TPID_VAL:n
 * if ifh.encap.tag_tipd > 0: 
 *   Custom TPID 1 to 3 configured by  REW::TPID_CFG:TPID_VAL:n
 *
...
/** 
 * \details
 * Based on configuration and the ifh.encap.prot_active field set by ANA it
 * is possible to change the ENCAP_ID.
 *
 ...
 * Sequence number will automatically increase at each transmisson refering
 * to the sequence entry.
 *
 * \details 
