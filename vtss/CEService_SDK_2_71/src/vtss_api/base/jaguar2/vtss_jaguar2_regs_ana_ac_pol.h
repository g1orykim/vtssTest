 ...
 *
 * \details 
 * When POL_PORT_CTRL:POL_PORT_CFG.FRAME_RATE_ENA is disabled burst
 * capacity is configured in steps of 8192 bytes.
 * 0: Always closed
 * 1: Burst capacity = 8192 bytes
 * n: Burst capacity = n x 8192 bytes
 * 63: Burst capacity = 516096 bytes
 * When POL_PORT_CTRL:POL_PORT_CFG.FRAME_RATE_ENA is enabled burst capacity
 * is configured in steps of 8192/2504 frames.
 * 0: Always closed
 * 1: Burst capacity = 1 x 8192/2504 frames
 * n: Burst capacity = n x 8192/2504 frames
 * 63: Burst capacity = 206 frames
 ...
 *
 * \details 
 * When POL_PORT_CTRL:POL_PORT_CFG.FRAME_RATE_ENA is disabled, policing is
 * performed in bits per second (bps). Policing rate is configured in units
 * of 25040 bps.
 * 0: Open until burst capacity is used, then closed.
 * 1: Rate = 25040 bps
 * n: Rate = n x 25040 bps
 * When POL_PORT_CTRL:POL_PORT_CFG.FRAME_RATE_ENA is enabled, policing is
 * performed in frames per second (fps). Policing rate is configured in
 * units of 10 fps.
 * 0: Open until burst capacity is used, then closed.
 * 1: Rate = 10 fps
 ...
 * \details 
 * 0: Reserved (Policer disabled).
 * 1 : Only frontport destinations are policed.
 * 2 : Only CPU destinations are policed.
 * 3 : Both frontports and/or CPU destinations are being policed.
 *
