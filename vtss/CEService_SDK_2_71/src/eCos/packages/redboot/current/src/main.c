...
#if defined(CYGNUM_REDBOOT_PERSIST_DATA_LENGTH) && (CYGNUM_REDBOOT_PERSIST_DATA_LENGTH > 0)
    persist_data_length = CYGNUM_REDBOOT_PERSIST_DATA_LENGTH;
    workspace_end -= persist_data_length; /* Reserve persisted memory */
    persist_data_start = (CYG_ADDRESS) workspace_end;
#else
    persist_data_length = 0;
    persist_data_start = (CYG_ADDRESS) NULL;
#endif
