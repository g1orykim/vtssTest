...
#define HAL_INTERRUPT_INFO( _vector_, _isr_, _data_, _object_ )         \
{                                                                       \
    cyg_uint32 _index_;                                                 \
    HAL_TRANSLATE_VECTOR( _vector_, _index_ );                          \
                                                                        \
    *(_isr_)    = (cyg_ISR *)hal_interrupt_handlers[_index_];           \
    *(_data_)   = hal_interrupt_data[_index_];                          \
    *(_object_) = (Cyg_Interrupt *)hal_interrupt_objects[_index_];      \
}
