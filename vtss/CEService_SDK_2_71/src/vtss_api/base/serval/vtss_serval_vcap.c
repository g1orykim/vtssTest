                                  ...
                                  srvl_tcam_data_t *data)
{
    ...
    VTSS_N("%s, row: %u, col: %u, key_size: %s",
            tcam->name, idx->row, idx->col, vtss_vcap_key_size_txt(idx->key_size));
                                     ...
                                     int bank, vtss_vcap_idx_t *idx, u32 *counter, BOOL clear) 
{
    ...
    VTSS_I("%s, row: %u, col: %u, key_size: %s, clear: %u",
            tcam->name, idx->row, idx->col, vtss_vcap_key_size_txt(idx->key_size), clear);
                                   ...
                                   srvl_tcam_data_t *data)
{
    ...
    VTSS_I("%s, row: %u, col: %u, key_size: %s, counter: %u",
            tcam->name, idx->row, idx->col, vtss_vcap_key_size_txt(idx->key_size), cnt);
                                    ...
                                    int bank, vtss_vcap_idx_t *idx, u32 count, BOOL up)
{
    ...
    VTSS_I("%s, row: %u, col: %u, key_size: %s, count: %u, up: %u",
            tcam->name, idx->row, idx->col, vtss_vcap_key_size_txt(idx->key_size), count, up);
...
static vtss_rc srvl_is2_action_set(vtss_state_t *vtss_state,
                                   srvl_tcam_data_t *data, vtss_acl_action_t *action, u32 counter)
{
                                  ...
                                  vtss_vcap_idx_t *idx, vtss_vcap_data_t *vcap_data, u32 counter)
{
    ...
    if (idx->key_size == VTSS_VCAP_KEY_SIZE_QUARTER) {
        ...
        /* Setup action */
        VTSS_RC(srvl_is2_action_set(vtss_state, data, &ace->action, counter));
                                    ...
                                    const vtss_acl_policer_no_t policer_no)
{
    ...
    return vtss_srvl_policer_conf_set(vtss_state, policer_no + SRVL_POLICER_ACL, &pol_conf);
}
                                      ...
                                      const vtss_port_no_t port_no)
{
    ...
    memset(&data, 0, sizeof(data));
    data.action_offset = tcam->action_type_width;
    VTSS_RC(srvl_is2_action_set(vtss_state, &data, &conf->action, 0));
    VTSS_RC(srvl_vcap_action2cache(vtss_state, tcam, &data));
    return srvl_vcap_port_cmd(vtss_state, tcam, port, VTSS_TCAM_CMD_WRITE, VTSS_TCAM_SEL_ACTION);
}
                                           ...
                                           BOOL                 dmac_dip_old)
{
    ...
    if ((key_new != key_old) || (dmac_dip_new != dmac_dip_old)) {
        ...
        vtss_vcap_key_size_t key_size_new = vtss_vcap_key_type_to_size(key_new);
        vtss_vcap_key_size_t key_size_old = vtss_vcap_key_type_to_size(key_old);
