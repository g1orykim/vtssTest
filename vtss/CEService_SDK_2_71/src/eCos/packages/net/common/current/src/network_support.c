...
#define CYG_NET_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))
...
cyg_bool_t init_loopback_interface(int lo)
{
    ...
    if (loopback_count >= CYGPKG_NET_NLOOP)
        return true;
    else
        loopback_count++;
...
init_all_network_interfaces(void)
{
    ...
    while ( in_init_all_network_interfaces ) {
        ...
        cyg_thread_delay(CYG_NET_MSEC2TICK(100));
        cyg_scheduler_lock();
    }
    ...
    if ( ! eth0_up ) { // Make this call idempotent
        ...
        if ((t = eth_drv_netdev("eth0")) != (cyg_netdevtab_entry_t *)NULL) {
            ...
            while (t->status != CYG_NETDEVTAB_STATUS_AVAIL) {
                ...
                cyg_thread_delay(CYG_NET_MSEC2TICK(1000));
            }
        }
#endif // CYGPKG_IO_PCMCIA
    ...
    if (rs_wait == 0 ) {
      ...
      cyg_thread_delay(CYG_NET_MSEC2TICK(2000));
    }
#endif
