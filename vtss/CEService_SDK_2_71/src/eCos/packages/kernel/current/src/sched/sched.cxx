...
extern  cyg_bool volatile threadload_started;
...
void Cyg_Scheduler::unlock_inner( cyg_ucount32 new_lock )
{
    ...
    do {
#ifdef CYGPKG_THREADLOAD
        cyg_bool called_pending_DSRs = false;
#endif
        ...
        if( new_lock == 0 && Cyg_Interrupt::DSRs_pending() ) {
            ...
            if (threadload_started) {
                called_pending_DSRs = true;
                threadload_update(0); // 0 means DSR
            }
#endif
...
#ifdef CYGPKG_THREADLOAD
                if (threadload_started) {
                    called_pending_DSRs = false; // Prevent the call to threadload_update() below.
                    threadload_update(next->get_unique_id());
                }
#endif
...
#ifdef CYGPKG_THREADLOAD
        if (threadload_started && called_pending_DSRs) {
            // Only here due to DSR calls. Gotta switch back to
            // original thread ID.
            threadload_update(current->get_unique_id());
        }
#endif
