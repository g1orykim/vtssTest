	...
	)
{
	 ...
	 */
	NTP_MD5Init(&md5);
	NTP_MD5Update(&md5, key, (u_int)cache_keylen);
	NTP_MD5Update(&md5, (u_char *)pkt, (u_int)length);
	NTP_MD5Final(digest, &md5);
...
addr2refid(struct sockaddr_storage *addr)
{
	...
	NTP_MD5Init(&md5);
	NTP_MD5Update(&md5, (u_char *)&GET_INADDR6(*addr),
