...
#define STRING_LEN	32
	...
	if (!(instance = (struct instance *)ntp_callout_malloc(sizeof *instance))) {
		perror("malloc");
		return (0);
	}
	...
	)
{
	...
	*/
	...
	if ((fd = open(instance->shmem_fname, O_RDONLY)) < 0)
		...
		if (shmem_old_size != 0) {
			shmem_old = (u_char *)ntp_callout_malloc((unsigned) sbuf.st_size);
			if (shmem_old == NULL)
	...
	buf = ntp_callout_malloc(shmem_length);
	if (buf == NULL) {
 ...
 */
	...
	while (fgets(line, 100, fd)) {
		...
		if (!strncmp(cc, "STATUS", (size_t) 6) || !strncmp(cc, "SHMEM", (size_t) 5)) {
			i = strlen(ca);
			instance->shmem_fname = (char *)ntp_callout_malloc((unsigned) (i+1));
