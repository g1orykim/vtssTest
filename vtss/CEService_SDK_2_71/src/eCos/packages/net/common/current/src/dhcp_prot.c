...
#define CYG_DHCP_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))
...
void dhcp_set_hostname(char *hostname)
{
    ...
    if (strlen(hostname)> CYGNUM_NET_DHCP_OPTION_HOST_NAME_LEN) {
        // trim the redundant string
        dhcp_hostname[CYGNUM_NET_DHCP_OPTION_HOST_NAME_LEN] = '\0';
    }
...
#ifdef CYGOPT_NET_DHCP_OPTION_HOST_NAME
{
    ...
    if (nlen > 0)
    	set_variable_tag( xmit, TAG_HOST_NAME, (cyg_uint8 *) dhcp_hostname, nlen + 1);

#if 1 /* Use host name as client ID */
    if (nlen > 0) {
        cyg_uint8 id[CYGNUM_NET_DHCP_OPTION_HOST_NAME_LEN+1];

        id[0] = 0;
        memcpy(&id[1], dhcp_hostname, nlen);
        set_variable_tag( xmit, TAG_DHCP_CLIENTID, id, nlen + 1);
    }
#endif
}
#endif
    ...
    while ( 1 ) {
        ...
        case DHCPSTATE_INIT:
            ...
            if(sendto(s, xmit, dhcp_size_for_send(xmit), 0, 
                ...
                perror("sendto");
                for (;;) {
                    cyg_thread_delay(CYG_DHCP_MSEC2TICK(1000));
                }
        ...
        cyg_uint8 *pstate, struct dhcp_lease *lease)
{
    if ( 0                           != *pstate
        ...
        *pstate = DHCPSTATE_DO_RELEASE;
        ...
        cyg_thread_delay(CYG_DHCP_MSEC2TICK(1000)); // to let it leave the building
    }
    return true;
}
