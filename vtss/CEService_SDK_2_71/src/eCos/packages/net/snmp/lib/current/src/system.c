...
void get_myipv6addr(int is_link_local, char* ipv6_string)
{
    ...
    while (ifp != (struct ifaddrs *)NULL) {
        if (ifp->ifa_addr->sa_family != AF_LINK &&
            ifp->ifa_name[0] == 'e' /* Consider eth0 only */) {
            ...
            if (ifp->ifa_addr->sa_family == AF_INET6) {
                if (is_link_local == 1 && (memcmp(&addr[0], "fe80", 4) == 0)) {
                    /* James: It is tricky. eCos uses this byte as the index of interface
                     * to indicate which interface this link local address assign
                     * to. It is called scope id in eCos.
                     * However, it is used internally. Just recover it.
                     */
                    addr[5] = '0';

                    memcpy(ipv6_string, addr, sizeof(addr));
                    return;
                }
                if (is_link_local == 0 && (memcmp(&addr[0], "fe80", 4) != 0)) {
                    memcpy(ipv6_string, addr, sizeof(addr));
                    return;
                }    
            }    
