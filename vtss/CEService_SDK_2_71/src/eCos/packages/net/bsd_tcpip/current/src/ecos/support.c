...
static int vtss_start_ips_netint = 0;
#ifdef CYGPKG_NET_INET6
extern void nd6_activate(int flag);
#endif /* CYGPKG_NET_INET6 */
void vtss_start_ips_netint_signal(int flag)
{
    if (!vtss_start_ips_netint)
        return;

    if (vtss_start_ips_netint == -1) {
#ifdef CYGPKG_NET_INET6
        nd6_activate(flag);
#endif /* CYGPKG_NET_INET6 */
        cyg_thread_resume(netint_thread_handle);    // Start it
        vtss_start_ips_netint = 1;
    }
}
#endif /* VTSS_IPS_IMPL */
...
#include <net/netdb.h>
#include <net/route.h>
externC char *if_indextoname(int indx, char *buf);
...
_mask6(struct sockaddr *sa, char *buf, int _len)
{
  ...
  while (addrword < 4) {
    ...
    HAL_LSBIT_INDEX(index, ntohl(sin6->sin6_addr.s6_addr32[addrword++]));
    bits += (32-index);
    if (index != 0) {
      break;
    }
...
static void
_show_ifp(struct ifnet *ifp, pr_fun *pr, char *if_name, int show_addr, int show_stat)
{
    struct ifaddr *ifa;
    ...
    TAILQ_FOREACH(ifa, &ifp->if_addrlist, ifa_list) {
        if (ifa->ifa_addr->sa_family == AF_LINK) {
            ...
            (*pr)("    link: %02x-%02x-%02x-%02x-%02x-%02x\n", ll[0], ll[1], ll[2], ll[3], ll[4], ll[5]);
            if (show_stat) {
                (*pr)("     RX: bytes  packets  errors  mcast   bcast\n");
                (*pr)("     %-10d %-8d %-7d %-7d %-7d\n",
                      ifp->if_ibytes, ifp->if_ipackets, ifp->if_ierrors, ifp->if_imcasts, ifp->if_ibcasts);
                (*pr)("     TX: bytes  packets  errors  mcast   bcast\n");
                (*pr)("     %-10d %-8d %-7d %-7d %-7d\n",
                      ifp->if_obytes, ifp->if_opackets, ifp->if_oerrors, ifp->if_omcasts, ifp->if_obcasts);
            }
        } else if (show_addr) {
...
#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
                if (ifa6->ia6_flags & IN6_IFF_DISABLED)   (*pr)("Disabled ");
#endif /* VTSS_IPS_IMPL */
            ...
            if (show_stat) {
                (*pr)("     RX: bytes  packets  errors  mcast   bcast\n");
                (*pr)("     %-10d %-8d %-7d %-7d %-7d\n",
                      ifa->if_ibytes, ifa->if_ipackets, ifa->if_ierrors, ifa->if_imcasts, ifa->if_ibcasts);
                (*pr)("     TX: bytes  packets  errors  mcast   bcast\n");
                (*pr)("     %-10d %-8d %-7d %-7d %-7d\n",
                      ifa->if_obytes, ifa->if_opackets, ifa->if_oerrors, ifa->if_omcasts, ifa->if_obcasts);
            }
        }
    }
}
...
void
show_network_interfaces(pr_fun *pr, char *if_name, int show_addr, int show_stat)
{
    struct ifnet *ifp;
    char          name[IFNAMSIZ];
    ...
    for (ifp = ifnet.tqh_first; ifp; ifp = ifp->if_link.tqe_next) {
        if (if_indextoname(ifp->if_index, name)) {
            if ((if_name == NULL) || strstr(name, if_name)) { /* if_name empty or matches real interface name */
                _show_ifp(ifp, pr, name, show_addr, show_stat);
            }
        }
    }
    cyg_scheduler_unlock();
...
{
    (*pr)("Route table:\n");
    show_network_routes(pr);

    (*pr)("\nInterfaces:\n");
    show_network_interfaces(pr, NULL, 1, 1);
}

#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
typedef void vpr_fun(int sid, char *fmt, ...);
...
#define sprtpr          (vprt.pr)
#define sprtsid         (vprt.sid)
#define sprtmore        (vprt.prmore)
#define pprtpr          (vprt->pr)
#define pprtsid         (vprt->sid)
#define pprtmore        (vprt->prmore)

#define SVTS_SHOW(...)  do {                                \
    if (sprtsid < 0) (*((pr_fun *)sprtpr))(__VA_ARGS__);    \
    else (*((vpr_fun *)sprtpr))(sprtsid, __VA_ARGS__);      \
} while (0);

#define PVTS_SHOW(...)  do {                                \
    if (pprtsid < 0) (*((pr_fun *)pprtpr))(__VA_ARGS__);    \
    else (*((vpr_fun *)pprtpr))(pprtsid, __VA_ARGS__);      \
} while (0);

static int _ips_dump_defrt6(struct radix_node *rn, void *vw)
{
    struct rtentry      *rt;
    struct sockaddr     *dst, *gate, *netmask;
    char                prtbuf[64], *bp;
    struct vtss_pr_fun  *vprt;
    ...
    dst = rt_key(rt);
    netmask = rt_mask(rt);
    if (!dst || !netmask || ((rt->rt_flags & (RTF_UP | RTF_WASCLONED)) != RTF_UP)) {
        return 0;
    }
    ...
    if (IN6_IS_ADDR_UNSPECIFIED(&((struct sockaddr_in6 *)dst)->sin6_addr)) {
        if (!IN6_IS_ADDR_UNSPECIFIED(&((struct sockaddr_in6 *)netmask)->sin6_addr)) {
            return 0;
        }
    } else {
        return 0;
#if 0
        if (!IN6_IS_ADDR_LINKLOCAL(&((struct sockaddr_in6 *)dst)->sin6_addr)) {
            return 0;
        }
    ...
    vprt = (struct vtss_pr_fun *)vw;
    gate = rt->rt_gateway;

    memset(prtbuf, 0x0, sizeof(prtbuf));
    if (!gate) {
        if (pprtmore) {
            PVTS_SHOW("\n\rIPv6 Router : ::");
        } else {
            PVTS_SHOW("IPv6 Router : ::");
        }
    } else {
        _inet_ntop(gate, prtbuf, sizeof(prtbuf));
        if (pprtmore) {
            PVTS_SHOW("\n\rIPv6 Router : %s", prtbuf);
        } else {
            PVTS_SHOW("IPv6 Router : %s", prtbuf);
        }
    }

    if (pprtmore) {
        bp = prtbuf;
        if ((rt->rt_flags & RTF_UP))        *bp++ = 'U';
        if ((rt->rt_flags & RTF_GATEWAY))   *bp++ = 'G';
        if ((rt->rt_flags & RTF_HOST))      *bp++ = 'H';
        if ((rt->rt_flags & RTF_REJECT))    *bp++ = '!';
        if ((rt->rt_flags & RTF_STATIC))    *bp++ = 'S';
        if ((rt->rt_flags & RTF_DYNAMIC))   *bp++ = 'D';
        if ((rt->rt_flags & RTF_MODIFIED))  *bp++ = 'M';
        *bp = '\0';

        PVTS_SHOW(" (Flags: %s", prtbuf);

        memset(prtbuf, 0x0, sizeof(prtbuf));
        if_indextoname(rt->rt_ifp->if_index, prtbuf);
        PVTS_SHOW(" / Interface: %s)\n", prtbuf);
    } else {
        PVTS_SHOW("\n");
    }
...
/*
    Caller should protect ifa via
    cyg_scheduler_lock()
    cyg_scheduler_unlock()
*/
#include <netinet6/nd6.h>
int set_ips_v6_info(struct in6_ifaddr *ifaddr6, int state)
{
    if (nd_ifinfo == NULL) {
        return (0);
    }
    ...
    if (state) {
        if (ifaddr6->ia6_flags & IN6_IFF_DISABLED) {
            ifaddr6->ia6_flags &= ~IN6_IFF_DISABLED;
            nd_ifinfo[ifaddr6->ia_ifp->if_index].flags &= ~ND6_IFF_IFDISABLED;
        }
    } else {
        if (!(ifaddr6->ia6_flags & IN6_IFF_DISABLED)) {
            ifaddr6->ia6_flags |= IN6_IFF_DISABLED;
...
void show_ips_v6_info(pr_fun *pr, int session, int prmore)
{
    struct ifnet        *ifp;
    struct ifaddr       *ifa;
    ...
    struct vtss_pr_fun  vprt;
    char                addr[64], netmask[64];
    int                 i, error, intf_cnt = 0;
    ...
    for (ifp = ifnet.tqh_first; ifp; ifp = ifp->if_link.tqe_next) {
        ...
        TAILQ_FOREACH(ifa, &ifp->if_addrlist, ifa_list) {
            ...
            _mask6(ifa->ifa_netmask, netmask, 64);
            if (sprtmore) {
                SVTS_SHOW("\n\rIPv6 Address: %s/%s Scope:", addr, netmask);
            } else {
                SVTS_SHOW("IPv6 Address: %s/%s Scope:", addr, netmask);
            }
            ipa6 = &ifa6->ia_addr.sin6_addr;
            if (IN6_IS_ADDR_MULTICAST(ipa6)) {
                if (IN6_IS_ADDR_MC_NODELOCAL(ipa6) ||
                    IN6_IS_ADDR_MC_INTFACELOCAL(ipa6)) {
                    SVTS_SHOW("Node ");
                }
                if (IN6_IS_ADDR_MC_LINKLOCAL(ipa6)) {
                    SVTS_SHOW("Link ");
                }
                if (IN6_IS_ADDR_MC_SITELOCAL(ipa6)) {
                    SVTS_SHOW("Site ");
                }
                if (IN6_IS_ADDR_MC_ORGLOCAL(ipa6)) {
                    SVTS_SHOW("Organization ");
                }
                if (IN6_IS_ADDR_MC_GLOBAL(ipa6)) {
                    SVTS_SHOW("Global ");
                }
            } else {
            ...
            if (ifa6->ia6_flags & IN6_IFF_DISABLED)     SVTS_SHOW("[Inactive] ");
            if (ifa6->ia6_flags & IN6_IFF_ANYCAST)      SVTS_SHOW("[Anycast] ");
            if (ifa6->ia6_flags & IN6_IFF_TENTATIVE)    SVTS_SHOW("[Tentative] ");
            if (ifa6->ia6_flags & IN6_IFF_DUPLICATED)   SVTS_SHOW("[Duplicate] ");
            if (ifa6->ia6_flags & IN6_IFF_DETACHED)     SVTS_SHOW("[Detached]");
            if (ifa6->ia6_flags & IN6_IFF_DEPRECATED)   SVTS_SHOW("[Deprecated] ");
            if (ifa6->ia6_flags & IN6_IFF_NODAD)        SVTS_SHOW("[NoDAD] ");
            if (ifa6->ia6_flags & IN6_IFF_AUTOCONF)     SVTS_SHOW("[AutoConf] ");
            if (ifa6->ia6_flags & IN6_IFF_TEMPORARY)    SVTS_SHOW("[Tempory] ");
            if (ifa6->ia6_flags & IN6_IFF_HOME)         SVTS_SHOW("[Home] ");

            SVTS_SHOW("Status:");
            if ((ifp->if_flags & IFF_UP))               SVTS_SHOW("UP/");
            if ((ifp->if_flags & IFF_LOOPBACK))         SVTS_SHOW("LOOPBACK/");
            if ((ifp->if_flags & IFF_POINTOPOINT))      SVTS_SHOW("P2P/");
            if ((ifp->if_flags & IFF_RUNNING)) {
                if (ifa6->ia6_flags & IN6_IFF_DISABLED) {
                    SVTS_SHOW("RUNNING%s/", sprtmore ? "(Disabled)" : "");
                } else {
                    SVTS_SHOW("RUNNING%s/", sprtmore ? "(Enabled)" : "");
                }
            }
            SVTS_SHOW("MTU %d/", ifp->if_mtu);
            SVTS_SHOW("LinkMTU is %d\n", nd_ifinfo[ifp->if_index].linkmtu);

            if (sprtmore) {
                SVTS_SHOW("Medium Mode:");
                if ((ifp->if_flags & IFF_PROMISC))          SVTS_SHOW(" PROMISC");
                if ((ifp->if_flags & IFF_BROADCAST))        SVTS_SHOW(" BROADCAST");
                if ((ifp->if_flags & IFF_MULTICAST))        SVTS_SHOW(" MULTICAST");
                if ((ifp->if_flags & IFF_ALLMULTI))         SVTS_SHOW(" ALLMULTI");
                SVTS_SHOW("\n");

                SVTS_SHOW("Drops:%d / Collisions:%d\n", ifa->if_iqdrops, ifa->if_collisions);
                SVTS_SHOW("RX: bytes      packets    errors     mcast      bcast\n");
                SVTS_SHOW("    %-10d %-10d %-10d %-10d %-10d\n",
                          ifa->if_ibytes,
                          ifa->if_ipackets,
                          ifa->if_ierrors,
                          ifa->if_imcasts,
                          ifa->if_ibcasts);
                SVTS_SHOW("TX: bytes      packets    errors     mcast      bcast\n");
                SVTS_SHOW("    %-10d %-10d %-10d %-10d %-10d\n",
                          ifa->if_obytes,
                          ifa->if_opackets,
                          ifa->if_oerrors,
    ...
    if (intf_cnt > 0) {
        ...
        for (i = 1; i <= AF_MAX; i++) {
            if ((rnh = rt_tables[i]) != NULL) {
                error = rnh->rnh_walktree(rnh, _ips_dump_defrt6, &vprt);
            }
        }
    }
    cyg_scheduler_unlock();
