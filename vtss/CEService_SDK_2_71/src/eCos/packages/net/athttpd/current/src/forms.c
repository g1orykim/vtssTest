...
cyg_httpd_handle_method_POST(void)
{
    ...
    if (httpstate.content_len == 0 || 
        ...
        cyg_httpd_send_error(httpstate.content_len == 0 ? CYG_HTTPD_STATUS_BAD_REQUEST : CYG_HTTPD_STATUS_TOO_LARGE);
        return;
    }
...
#ifdef CYGOPT_NET_ATHTTPD_HTTPS
            if (ssl)
                len = SSL_read(ssl, httpstate.post_data + post_data_len, httpstate.content_len - post_data_len);
            else
#endif /* CYGOPT_NET_ATHTTPD_HTTPS */
                len = read(descr, httpstate.post_data + post_data_len, httpstate.content_len - post_data_len);
            if(len < 0)
                goto out;     // Socket broken while processing request
            post_data_len += len;
        }
    }
    /* peter, 2008/11, in IE browser, the post date length maybe include CRLF
    CYG_ASSERT(post_data_len == httpstate.content_len, "Partial read"); */
    if(post_data_len > httpstate.content_len + 2)
        goto out;

    // httpstate.post_data remains available in handler
