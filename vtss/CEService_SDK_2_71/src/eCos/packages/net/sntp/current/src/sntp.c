 ...
 * the number of hostnames in the array.
   peter, 2010/11/30, don't use point for 'sntp_servers'
static sntp_server_list_t *sntp_servers = NULL; */
#define SNTP_SERVER_MAX_CNT     2   //one is for IPv4 and another is for IPv6
static sntp_server_list_t sntp_servers[SNTP_SERVER_MAX_CNT];
 ...
 */
#define	CYGNUM_SNTP_STACK_SIZE_MIN	8192
...
#define CYG_SNTP_MSEC2TICK(msec) (msec / (CYGNUM_HAL_RTC_NUMERATOR / CYGNUM_HAL_RTC_DENOMINATOR / 1000000))
...
static void sntp_fn(cyg_addrword_t data)
{
  ...
  // Use random source port for NTP client
  //local.sin_port = serv->s_port;
  local.sin_addr.s_addr = INADDR_ANY;

  while (bind(fd,(struct sockaddr *)&local,sizeof(local)) < 0) {
      cyg_thread_delay(CYG_SNTP_MSEC2TICK(3000));      
  }
  ...
  CYG_ASSERT(-1 != fd6,"Failed to open socket");
...
#else
    /* Bugzilla#1743 sntp deadlock */  
  while (bind(fd6, (struct sockaddr *)&local6,sizeof(local6)) < 0) {
      cyg_thread_delay(CYG_SNTP_MSEC2TICK(3000));      
  }
#endif  
  ...
  while (1) {
...
#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
            /*
                Send an NTP request to each NTP server in our server list.
                Use corresponding version number for IPv4/IPv6 respectively.
            */
            memset(&ntp_pkt, 0, sizeof(ntp_pkt));
#endif /* VTSS_IPS_IMPL */
            ...
            if (sntp_servers[i].ip_sntp_server.sa_family == AF_INET)
            {
                struct sockaddr_in host;

#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
                ntp_pkt.Control = NTP_LI_NOLEAP |
                                  NTP_MODE_SET(NTP_MODE_CLIENT) |
                                  NTP_VERSION_SET(3);
#endif /* VTSS_IPS_IMPL */
                ...
                error = getaddrinfo((char *)sntp_servers[i].ip_sntp_server_string, "ntp", &hints, &res);
                if(error) {
                    #ifdef VTSS_SW_OPTION_SYSLOG
                    S_W("DNS loopup fail for SNTP server (%s)", sntp_servers[i].ip_sntp_server_string);
                    #endif
                }
                else {
                    memmove(&host, res->ai_addr, res->ai_addrlen);
                    freeaddrinfo(res);
                    memcpy(&sntp_servers[i].ip_sntp_server, &host, sizeof(struct sockaddr_in));
                    struct sockaddr_in *saddr = (struct sockaddr_in *)&sntp_servers[i].ip_sntp_server;
                    struct servent         *service;
                    service = getservbyname("ntp", "udp");
                    saddr->sin_port = service->s_port;  /* Already network-endian */                       
                sendto(fd, &ntp_pkt, sizeof(ntp_pkt), 0,
                        &sntp_servers[i].ip_sntp_server, sntp_servers[i].ip_sntp_server.sa_len);
                }
#ifdef CYGPKG_NET_INET6
            } else if (sntp_servers[i].ip_sntp_server.sa_family == AF_INET6) {
#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
                ntp_pkt.Control = NTP_LI_NOLEAP |
                                  NTP_MODE_SET(NTP_MODE_CLIENT) |
                                  NTP_VERSION_SET(4);
    ...
    latest_fraction   = ntohl(ntp_pkt.TransmitTimestamp.Fraction);
    ...
    if (is_better(&new_srv,&best_srv)) {
      ...
      if (diff > 2) {
        ...
        struct timeval tv;
        tv.tv_sec  = new_time;
        tv.tv_usec = (1000000ULL * latest_fraction) / 4294967296ULL;
...
/*
 * Get the SNTP Poll interval 
 */
int get_sntp_poll_interval()
{
  return SNTP_UPDATEPERIOD;
}
 ...
 *		None
 */
void cyg_sntp_set_servers(sntp_server_list_t *server_list,
        cyg_uint32 num_servers)
{
#if !defined(VTSS_SW_OPTION_NTP)
    cyg_uint32 i = 0;

    CYG_ASSERT(num_servers <= SNTP_SERVER_MAX_CNT, "cyg_sntp_set_servers");
...
void cyg_sntp_get_servers(sntp_server_list_t *server_list,
                          cyg_uint32 *num_servers)
{
    cyg_uint32 i = 0;
    ...
    if (sntp_initialized) {
        ...
        /* Copy the list */
        for (i = 0; i < sntp_num_servers && i < *num_servers; i++)
            server_list[i] = sntp_servers[i];
