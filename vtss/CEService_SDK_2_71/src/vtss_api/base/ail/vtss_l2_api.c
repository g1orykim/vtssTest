                          ...
                          const vtss_port_no_t port_no)
{
    ...
    if ((rc = vtss_inst_check(inst, &vtss_state)) == VTSS_RC_OK) {
        if (port_no >= vtss_state->port_count && port_no != VTSS_PORT_NO_NONE) {
            ...
            rc = VTSS_RC_ERROR;
        } else if (vid > VTSS_VID_RESERVED) {
            VTSS_E("illegal vid: %u", vid);
            rc = VTSS_RC_ERROR;
        } else {
            rc = VTSS_FUNC(port.vid2port_set, vid, port_no);
        }
    }
    VTSS_EXIT();
                           ...
                           const vtss_lport_no_t lport_no)
{
    ...
    if ((rc = vtss_inst_check(inst, &vtss_state)) == VTSS_RC_OK) {
        ...
        rc = VTSS_FUNC(port.vid2lport_set, vid, lport_no);
    }
    return rc;
}
#endif /* VTSS_ARCH_B2 */
...
static vtss_rc vtss_port_no_none_check(vtss_state_t         *vtss_state,
                                       const vtss_port_no_t port_no)
{
                          ...
                          BOOL src_update, BOOL dest_update, BOOL aggr_update)
{
    ...
    for (i_port = VTSS_PORT_NO_START; i_port < port_count; i_port++) {
        ...
        tx_forward[i_port] = (vtss_state->l2.port_state[i_port] &&
                              ...
                              !vtss_state->port.conf[i_port].power_down);
    ...
    for (glag_no = VTSS_GLAG_NO_START; dest_update && glag_no < VTSS_GLAG_NO_END; glag_no++) {
        ...
        if (vtss_state->arch == VTSS_ARCH_L28) {
            u32            pgid_dest, pgid_src, pgid_aggr_a, pgid_aggr_b, ndx;
            pgid_dest = (vtss_state->l2.pgid_glag_dest + glag_idx);
            pgid_src = (vtss_state->l2.pgid_glag_src + glag_idx);
            pgid_aggr_a = (vtss_state->l2.pgid_glag_aggr_a + glag_idx);
            pgid_aggr_b = (vtss_state->l2.pgid_glag_aggr_b + glag_idx);
            for (port_no = VTSS_PORT_NO_START; port_no < port_count; port_no++) {
                vtss_state->l2.pgid_table[pgid_dest].member[port_no] = 0;
                vtss_state->l2.pgid_table[pgid_src].member[port_no] = 1;
                vtss_state->l2.pgid_table[pgid_aggr_a].member[port_no] = 0;
                vtss_state->l2.pgid_table[pgid_aggr_b].member[port_no] = 0;
            }

            /* Setup GLAG source and destination masks */
            for (gport = VTSS_GLAG_PORT_START; gport < VTSS_GLAG_PORT_END; gport++) {
                if ((port_no = vtss_state->l2.glag_members[glag_idx][gport]) == VTSS_PORT_NO_NONE)
                    break;
                
                /* Include local ports and stack ports in destination mask */
                vtss_state->l2.pgid_table[pgid_dest].member[port_no] = 1;
                
                /* Include locally aggregated stack ports in destination mask */
                if ((aggr_no = vtss_state->l2.port_aggr_no[port_no]) != VTSS_AGGR_NO_NONE &&
                    vtss_state->l2.vstax_port_enabled[port_no]) {
                    for (i_port = VTSS_PORT_NO_START; i_port < port_count; i_port++)
                        if (vtss_state->l2.port_aggr_no[i_port] == aggr_no)
                            vtss_state->l2.pgid_table[pgid_dest].member[i_port] = 1;
                }
                
                /* Exclude local ports from source mask */
                if (!vtss_state->l2.vstax_port_enabled[port_no]) 
                    vtss_state->l2.pgid_table[pgid_src].member[port_no] = 0;
            ...
            for (ac = 0; gcount && ac < vtss_state->l2.ac_count; ac++) {
                ndx = 0;
                for (gport = VTSS_GLAG_PORT_START; gport < VTSS_GLAG_PORT_END; gport++) {
                    if ((port_no = vtss_state->l2.glag_members[glag_idx][gport]) == VTSS_PORT_NO_NONE)
                        break;
                    ...
                    if (tx_forward[port_no]) {
                        ...
                        if (vtss_state->l2.vstax_port_enabled[port_no] &&
                            ((ndx + ac) % gcount) == 0) {
                            pgid = (((1 << VTSS_CHIP_PORT(port_no)) &
                                     vtss_state->l2.vstax_info.chip_info[0].mask_a) ? pgid_aggr_a : pgid_aggr_b);
                            vtss_state->l2.pgid_table[pgid].member[ac] = 1;
                            
                            /* If stack ports are aggregated, enable AC in both masks */
                            if (vtss_state->l2.port_aggr_no[port_no] != VTSS_AGGR_NO_NONE) {
    ...
    for (gport = VTSS_GLAG_PORT_START; gport < VTSS_GLAG_PORT_END; gport++) {
        ...
        for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count; port_no++) {
            ...
            if (vtss_state->l2.vstax_info.upsid[map->chip_no] == entry[gport].upsid &&
                map->chip_port == port && !vtss_state->l2.vstax_port_enabled[port]) {
...
#if defined(VTSS_FEATURE_VSTAX_V1)
static vtss_rc vtss_glag_set(vtss_state_t *vtss_state,
                             vtss_glag_no_t        glag_no,
                             const u32  member[VTSS_GLAG_PORT_ARRAY_SIZE])
{
    ...
    for (gport = VTSS_GLAG_PORT_START; gport < VTSS_GLAG_PORT_END; gport++) {
        ...
        vtss_state->l2.glag_members[glag_no-VTSS_GLAG_NO_START][gport] = port_no;
        if (port_no == VTSS_PORT_NO_NONE) {
            /* End of list */
            if (gport == VTSS_GLAG_PORT_START) /* Deleting GLAG, flush MAC table */
                VTSS_RC(vtss_mac_glag_flush(vtss_state, glag_no));
                           ...
                           const u32 member[VTSS_GLAG_PORT_ARRAY_SIZE])
{
    ...
    VTSS_ENTER();
    if ((rc = vtss_inst_check(inst, &vtss_state)) == VTSS_RC_OK)
        rc = vtss_glag_set(vtss_state, glag_no, member);
    VTSS_EXIT();
    return rc;
}
#endif /* VTSS_FEATURE_VSTAX_V1 */
                                 ...
                                 const vtss_vstax_port_conf_t *const conf)
{
    ...
    if ((rc = vtss_inst_chip_no_check(inst, &vtss_state, chip_no)) == VTSS_RC_OK) {
        ...
        rc = VTSS_FUNC(l2.vstax_port_conf_set, stack_port_a);
    }
    VTSS_EXIT();
    return rc;
}
...
#if defined(VTSS_FEATURE_VSTAX_V1)
            vtss_state->l2.glag_members[glag_no][VTSS_GLAG_PORT_START] = VTSS_PORT_NO_NONE;
#endif /* VTSS_FEATURE_VSTAX_V1 */
#if defined(VTSS_FEATURE_VSTAX_V2)
...
vtss_rc vtss_cmn_vlan_port_conf_set(vtss_state_t *vtss_state, const vtss_port_no_t port_no)
{
    ...
    vtss_mirror_conf_t     *mirror_conf = &vtss_state->l2.mirror_conf;
    vtss_vid_t             uvid;
                         ...
                         const vtss_vce_t     *const vce)
{
    ...
    if (vtss_vcap_lookup(vtss_state, is1_obj, is1_user, vce->id, &data, NULL) == VTSS_RC_OK) {
    ...
    /* Add main entry */
    VTSS_RC(vtss_vcap_add(vtss_state, is1_obj, is1_user, vce->id, vce_id, &data, 0));
    return VTSS_RC_OK;
}
...
vtss_rc vtss_cmn_vce_del(vtss_state_t *vtss_state, const vtss_vce_id_t vce_id)
{
    ...
    if (vtss_vcap_lookup(vtss_state, obj, VTSS_IS1_USER_VCL, vce_id, &data, NULL) != VTSS_RC_OK) {
    ...
    VTSS_RC(vtss_vcap_del(vtss_state, obj, VTSS_IS1_USER_VCL, vce_id));
                                     ...
                                     const vtss_vid_t trans_vid, const u8 *ports)
{
    ...
    /* Add main entry */
    VTSS_RC(vtss_vcap_add(vtss_state, is1_obj, is1_user, vte_id, VTSS_VCAP_ID_LAST, &data, 0));
    return VTSS_RC_OK;
}
...
static vtss_rc vtss_vt_is1_entry_del(vtss_state_t *vtss_state, const u64 vte_id)
{
    ...
    if (vtss_vcap_lookup(vtss_state, obj, VTSS_IS1_USER_VLAN, vte_id, &data, NULL) != VTSS_RC_OK) {
    ...
    VTSS_RC(vtss_vcap_range_free(&vtss_state->vcap.range, is1->sport_range));
    VTSS_RC(vtss_vcap_range_free(&vtss_state->vcap.range, is1->dport_range));
    VTSS_RC(vtss_vcap_del(vtss_state, obj, VTSS_IS1_USER_VLAN, vte_id));
    return VTSS_RC_OK;
}
                                         ...
                                         const u16 group_id, const u8 *ports)
{
    ...
    VTSS_RC(vcap_res_check(&vtss_state->vcap.is1.obj, is1_add, is1_del));
    VTSS_RC(vcap_res_check(&vtss_state->vcap.es0.obj, es0_add, es0_del));
    return VTSS_RC_OK;
}
                                      ...
                                      const u16 group_id, const vtss_vid_t vid, const vtss_vid_t trans_vid)
{
    ...
    if (ports_exist == TRUE) {
        ...
        if (entry_exist == FALSE) { /* If an entry already exists, we just need to update the entry */
            VTSS_RC(vcap_res_check(&vtss_state->vcap.is1.obj, 1, 0));
            VTSS_RC(vcap_res_check(&vtss_state->vcap.es0.obj, port_cnt, 0));
        }
    }
                                  ...
                                  const vtss_debug_info_t   *const info)
{
    ...
    for (vid = VTSS_VID_NULL; vid < VTSS_VIDS; vid++) {
        ...
        if (!entry->enabled && !info->full)
            continue;
        if (header)
            vtss_debug_print_port_header(vtss_state, pr, "VID   MSTI  Lrn  Mir  Iso  ", 0, 1);
        header = 0;
        pr("%-6u%-6u%-5u%-5u%-5u", vid, entry->msti, entry->conf.learning, entry->conf.mirror, entry->isolated);
