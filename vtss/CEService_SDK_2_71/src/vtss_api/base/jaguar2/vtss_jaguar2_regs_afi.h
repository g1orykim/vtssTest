 ...
 * 
 * If enabled in STICK_INFO_ENA.FRM_OUT_NEG_INFO_ENA, the corresponding
 ...
 * successively injected to queues of the same port or if the port
 * flow-controlled or is shaped to a low bandwidth.
 * 
 ...
 * PORT_TBL:PORT_FRM_OUT.FRM_OUT_MAX.
 * 
 * If enabled in STICK_INFO_ENA.FRM_OUT_MAX_INFO_ENA, the corresponding
 * port number is stored in STICKY_INFO.PORT_NUM.
 *
 * \details 
 ...
 * 
 * If enabled in STICK_INFO_ENA.ENQ_STOP_INFO_ENA, the affected port number
 * is stored in STICKY_INFO.PORT_NUM.
 *
 * \details 
 ...
 * TTI_MISC.TTI_CAL_SLOT_PTRS).
 * 
 * If enabled in STICK_INFO_ENA.TTI_BUSY_INFO_ENA, the corresponding TTI
...
/** 
 * \brief
 * Number of times STICK_INFO.PORT_NUM and  STICK_INFO.TTI is allowed to be
 * updated.
 ...
 * 
 * FRM_RM:
 * When set, next frame injection will cause frame to be removed from
 * buffer memory. 
 * This injection will not be transmitted on the destination port.
 * Once removed, HW will set FRM_GONE=1.
 * 
 * FRM_GONE:
 * Set by AFI when frame has been removed from buffer memory.
 * 
 * FRM_INFO:
 * Frame information, ref. MISC:NEW_FRM_INFO.FRM_INFO.
 *
 * \details 
 * Delay entry type:
 * Bit 0-29: DELAY
 ...
 * 
 * By setting TRAILING_DELAY_SEQ_CNT=N a trailing delay will only be
 * applied only for every Nth sequence injection.
 * 
 ...
 * flow-controlled.
 * 
 * The configured value will be counted down in DTI_FC_CNT_DOWN and when
 * zero is reached, another injection attempt will be made. In the meantime
 * DTI_CNT_DOWN will go negative, such that the additional delay will be
 * subtracted from the next delay in the DTI sequence.
 *
 * \details 
...
/** 
 * \brief
 * Port number that frames shall be injected on.
 *
 * \details 
 ...
 * memory. Upon injection TIMER_LEN will then be set to 0 (=Disable).
 * 
 ...
 * TICK_CNT is being decremented down by AFI, each time the tick - selected
 * by TICK_IDX - changes era, i.e. LAST_TICK_ERA differs from
 * TTI_TICKS.TTI_TICK_STATE.TICK_ERA.
 * 
...
/** 
 * \brief
 * Enable counting of injected frames in TTI_MISC:TTI_INJ_CNT.TTI_INJ_CNT.
 *
 * \details 
 ...
 * will toggle and the tick will restart counting from 0.
 * 
 ...
 * 
 * Note that writes will need two accesses and may thus have to await 2x
 * the configured number of clock cycles.
 * 
...
/** 
 * \brief
 * See PORT_TBL:PORT_FRM_OUT.FRM_OUT_MAX.
 *
 * \details 
 ...
 * 
 * DTI injections will be held back if FRM_OUT_MAX is exceeded.
 * TTI injections will be held back if FRM_OUT_MAX+TTI_FRM_OUT_MAX is
 * exceeded.
 * 
 * TTI_FRM_OUT_MAX ensures that TTI injections will still be possible for
 * when a DTI flow is configured slightly above port speed.
 * 
