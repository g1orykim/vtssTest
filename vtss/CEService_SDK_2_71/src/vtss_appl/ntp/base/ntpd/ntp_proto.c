...
clock_select(void)
{
	...
	if (nlist > list_alloc) {
		if (list_alloc > 0) {
			ntp_callout_free(endpoint);
			ntp_callout_free(indx);
			ntp_callout_free(peer_list);
		}
