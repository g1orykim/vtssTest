	...
	)
{
	...
	*/
	...
	temp_datum_pts_unit = (struct datum_pts_unit **)
		ntp_callout_malloc((nunits+1)*sizeof(struct datum_pts_unit *));
	...
	ntp_callout_free(datum_pts_unit);
	datum_pts_unit = temp_datum_pts_unit;
	datum_pts_unit[nunits] = (struct datum_pts_unit *)
		ntp_callout_malloc(sizeof(struct datum_pts_unit));

#if 1 /* CP: avoid use NULL */
    if ( datum_pts_unit[nunits] == NULL ) {
        msyslog(LOG_ERR, "datum_pts_start: memory insufficient");
        ntp_callout_free( temp_datum_pts_unit );
	...
	for (i=0; i<nunits; i++) {
		if (datum_pts_unit[i]->unit == unit) {
			...
			*/
			...
			ntp_callout_free(datum_pts_unit[i]);
