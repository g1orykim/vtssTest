...
#ifdef VTSS_DNS_ENHANCEMENT
/* Used to check host entry existed in cache table or not.
   if yes, it will return the idx of the entry.
   Otherwise, it will return -1. 
 */
static int cyg_host_tab_cache_check(const char *hostname) 
{
    int i, found_flag = -1;
    char host_name[256];
    int len;
    ...
    for(i=0; i<MAX_DNS_CACHED_ENTRIES; i++) {
        if(host_tab[i].valid == 1) {
            if(host_tab[i].length_of_qname != strlen(host_name))
                continue;
            if(strcmp(host_tab[i].query_host_name, host_name) == 0) {
                found_flag = 1;
                break;
            }
        }
...
/* Used to aging host cahche entry.
 */
void cyg_host_cache_tab_aging(int tick) 
{
    int i;
    
    for(i=0; i<MAX_DNS_CACHED_ENTRIES; i++) {
        if(host_tab[i].valid == 1) {
            if((host_tab[i].ttl - tick) > 0)
                host_tab[i].ttl = host_tab[i].ttl - tick;
            else {
                host_tab[i].valid = 0;
            }
        }
...
parse_answer(unsigned char * msg, short rr_type)
{
...
#ifdef VTSS_DNS_ENHANCEMENT
    int i, found_available_entry_flag;
    char    query_host_name[256];
    char *name_temp;
#endif //VTSS_DNS_ENHANCEMENT
...
#ifdef VTSS_DNS_ENHANCEMENT
    qname = ptr;
    name_temp = real_name(msg, qname);
    strcpy(query_host_name, name_temp);
    free_string(name_temp);
#endif //VTSS_DNS_ENHANCEMENT
    ...
    if (dns_hdr->ancount > 0) {
        ...
        case DNS_TYPE_A:
            ...
            hent->h_length = sizeof(struct in_addr);
#ifdef VTSS_DNS_ENHANCEMENT
            /* add it to the DNS cached table */
            if(cyg_host_tab_cache_check(query_host_name) == -1) {
                found_available_entry_flag = 0;
                for(i=0; i<MAX_DNS_CACHED_ENTRIES; i++) {
                    if(host_tab[i].valid == 0) {
                        found_available_entry_flag = 1;
                        break;
                    }
                }
                if(found_available_entry_flag) {
                    strcpy(host_tab[i].query_host_name, query_host_name);
                    strcpy(host_tab[i].real_host_name, hent->h_name);
                    memcpy(&host_tab[i].ip_addr, hent->h_addr_list[0], sizeof(struct in_addr));
                    host_tab[i].ttl = htonl(rr.ttl);
                    host_tab[i].length_of_qname = strlen(query_host_name);
                    host_tab[i].valid = 1;
                }
            }
#endif //VTSS_DNS_ENHANCEMENT
...
do_query(const char * hostname) 
{
    ...
    int index;
    struct hostent *hent;
    index = cyg_host_tab_cache_check(hostname);
    if(index != -1) {
        hent = alloc_hent();
        if (hent){
            hent->h_name = alloc_string(strlen(host_tab[index].real_host_name)+1);
            strcpy(hent->h_name, host_tab[index].real_host_name);
            memcpy(hent->h_addr_list[0], &host_tab[index].ip_addr, sizeof(struct in_addr));
            hent->h_addrtype = AF_INET;
            hent->h_length = sizeof(struct in_addr);
            return hent;                      
        }
...
#ifdef VTSS_DNS_ENHANCEMENT
void cyg_get_dns_query_name(unsigned char *dns_header_ptr, unsigned char *current_ptr, unsigned char *name)
{
    char *name_temp;
    name_temp = real_name(dns_header_ptr, current_ptr);
    strcpy((char *) name, name_temp);
    free_string(name_temp);
}
#endif //VTSS_DNS_ENHANCEMENT
