...
int phy_rd_wr(cyg_uint8 read,
              cyg_uint32 miim_controller,
              cyg_uint8 miim_addr,
              cyg_uint8 addr,
              cyg_uint16 *value)
{
    ...
    VTSS_DEVCPU_GCB_MIIM_MII_CMD(miim_controller) = data;
    
    /* Wait for MIIM operation to finish */
    do {
        data = VTSS_DEVCPU_GCB_MIIM_MII_STATUS(miim_controller);
    } while(data & VTSS_F_DEVCPU_GCB_MIIM_MII_STATUS_MIIM_STAT_BUSY);

    if (read) {
        data = VTSS_DEVCPU_GCB_MIIM_MII_DATA(miim_controller);
        if (data & VTSS_M_DEVCPU_GCB_MIIM_MII_DATA_MIIM_DATA_SUCCESS) {
            //diag_printf("Read(%d, %d) returned 0x%08x\n", miim_addr, addr, data);
            return -1;
        }
...
int vtss_phy_rd(cyg_uint32 miim_controller,
                cyg_uint8 miim_addr,
                cyg_uint8 addr,
                cyg_uint16 *value)
{
    if(phy_rd_wr(1, miim_controller, miim_addr, addr, value) == 0)
        return 0;
    diag_printf("Read(%d, %d) returned error\n", miim_addr, addr);
    return -1;
}

int vtss_phy_wr(cyg_uint32 miim_controller,
                cyg_uint8 miim_addr,
                cyg_uint8 addr,
                cyg_uint16 value)
{
    return phy_rd_wr(0, miim_controller, miim_addr, addr, &value);
}
...
void _cyg_plf_redboot_startup(void)
{
    ...
    if(chipid == 0x7428 || chipid == 0x7424) { /* Lu10 */
        gpio_out(29);               /* PWM used for LED dimming */
        gpio_set(29, 0);            /* nOE => Output enable */

        green_port = red_port = 26;
        green_bit = 0;
        red_bit = 1;
        bit_count = 2;

        VTSS_DEVCPU_GCB_SIO_CTRL_SIO_PORT_ENABLE = 0xFFF000FF;
        ...
        VTSS_DEVCPU_GCB_SIO_CTRL_SIO_PORT_ENABLE = 0xFFFFFFFF;

        cyg_uint16 val;

        /* Set PHY3.LED1 = GPIO8 = ON
         * Encoding:
         *
         * 15:12 = LED3
         * 11:8  = LED2
         ...
         * 3:0   = LED0
         *
         * This enables PWM output (via GPIO8), which is a
         * preconditions for *any* LEDs to light up. (Until we have
         * the "full" PWM mode up with the PHY etc.)
         */
        val = (15 << 4);            /* LED1 mode 15 = ON */
        vtss_phy_wr(1, 3, 29, val); /* Set LED mode PHY 3 */
...
static int tcamtest(bool show_progress)
{
    ...
    /*
     * NOTE: Needless to say TCAM BIST operations in general will
     * wreck havoc on a warm-start attempt.
     */

    return 
        run_tcam_bist(&VTSS_VCAP_IS0_TCAM_BIST_TCAM_CTRL, &VTSS_VCAP_IS0_TCAM_BIST_TCAM_STAT, "IS0") &&
        run_tcam_bist(&VTSS_VCAP_IS1_TCAM_BIST_TCAM_CTRL, &VTSS_VCAP_IS1_TCAM_BIST_TCAM_STAT, "IS1") &&
        run_tcam_bist(&VTSS_VCAP_IS2_TCAM_BIST_TCAM_CTRL, &VTSS_VCAP_IS2_TCAM_BIST_TCAM_STAT, "IS2") &&
        run_tcam_bist(&VTSS_VCAP_ES0_TCAM_BIST_TCAM_CTRL, &VTSS_VCAP_ES0_TCAM_BIST_TCAM_STAT, "ES0") &&
        run_tcam_bist(&VTSS_ANA_L3_2_TCAM_BIST_TCAM_CTRL, &VTSS_ANA_L3_2_TCAM_BIST_TCAM_STAT, "L3 "); 
    ...
    return 
        run_tcam_bist(&VTSS_VCAP_CORE_TCAM_BIST_TCAM_CTRL(VTSS_TO_S1), &VTSS_VCAP_CORE_TCAM_BIST_TCAM_STAT(VTSS_TO_S1), "IS1") &&
        run_tcam_bist(&VTSS_VCAP_CORE_TCAM_BIST_TCAM_CTRL(VTSS_TO_S2), &VTSS_VCAP_CORE_TCAM_BIST_TCAM_STAT(VTSS_TO_S2), "IS2") &&
        run_tcam_bist(&VTSS_VCAP_CORE_TCAM_BIST_TCAM_CTRL(VTSS_TO_ES0), &VTSS_VCAP_CORE_TCAM_BIST_TCAM_STAT(VTSS_TO_ES0), "ES0");
#else
    return 1;
#endif
}
