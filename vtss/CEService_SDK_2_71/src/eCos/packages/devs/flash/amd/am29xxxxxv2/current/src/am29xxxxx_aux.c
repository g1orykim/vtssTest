...
static int  AM29_RAMFNDECL(am29_hw_program_buffered, (volatile AM29_TYPE*, volatile AM29_TYPE*, const cyg_uint8*, cyg_uint32 count, int retries, cyg_uint32 wr_buf_sz));
...
AM29_FNNAME(am29_hw_cfi)(struct cyg_flash_dev* dev, cyg_am29xxxxx_dev* am29_dev, volatile AM29_TYPE* addr)
{
    ...
    int i;
    int erase_regions;
    cyg_uint32 wr_buf_sz_lsb, wr_buf_sz_msb;
...
//    amd_dump_buf( addr, 256 );
    ...
    // Write buffer sizes are always a power of 2.
    // The following code will cause the wr_buf_sz to become 1 if it's not
    // supported, but this is taken care of during the program operations
    // so that values 0 and 1 are treated identically.
    wr_buf_sz_lsb = AM29_UNSWAP(addr[AM29_OFFSET_CFI_WR_BUF_SZ_LSB]) & 0x00FF;
    wr_buf_sz_msb = AM29_UNSWAP(addr[AM29_OFFSET_CFI_WR_BUF_SZ_MSB]) & 0x00FF;
    am29_dev->wr_buf_sz = 0x01 << ((wr_buf_sz_msb << 8) | wr_buf_sz_lsb);

    // CFI returns the write buffer size in bytes, but this driver must have
    // it in AM29_TYPE quantities, corrected for number of devices in parallel
    // (assuming that all devices have the same write buffer size).
    am29_dev->wr_buf_sz /= (sizeof(AM29_TYPE) / AM29_DEVCOUNT);
...
// as per the bus alignment, not in bytes.
static int
AM29_FNNAME(am29_hw_program_buffered)(volatile AM29_TYPE* block_start, volatile AM29_TYPE* addr, const cyg_uint8* buf, cyg_uint32 count, int retries, cyg_uint32 wr_buf_sz)
{
    int i;

    // The mask will always be [write buffer size measured in bytes for a particular flash device - 1].
    cyg_uint32 mask = wr_buf_sz * (sizeof(AM29_TYPE) / AM29_DEVCOUNT) - 1;

    AM29_2RAM_ENTRY_HOOK();

    while (count > 0 && retries > 0) {
        volatile AM29_TYPE *last_addr;
        AM29_TYPE datum = 0, current, active_dq7s;
        
        // Compute number of words to write. All writes to the write
        // buffer must fall within the same write buffer page.
        cyg_uint32 nwords = wr_buf_sz - (((cyg_uint32)(addr) & mask) / (sizeof(AM29_TYPE) / AM29_DEVCOUNT));
        if (nwords > count)
            nwords = count;
        ...
        block_start[AM29_OFFSET_COMMAND2] = AM29_COMMAND_SETUP2;
        HAL_MEMORY_BARRIER();
        block_start[0]                    = AM29_COMMAND_LOAD_BUFFER;
        HAL_MEMORY_BARRIER();
        block_start[0]                    = AM29_SWAP(AM29_PARALLEL((nwords - 1))); // Program same count into all devices
        HAL_MEMORY_BARRIER();

        // Load data into write buffer
        for(i = 0; i < nwords; i++) {
            *addr++ = datum = AM29_NEXT_DATUM(buf);
        }
        last_addr = addr - 1;

        // Program buffer to flash
        block_start[0] = AM29_COMMAND_PROGRAM_BUFFER;
        ...
        while (--retries > 0) {
            ...
            // No DQ5 bits set in chips which are still programming. Poll again.
        }   // Retry for current write buffer
...
AM29_FNNAME(cyg_am29xxxxx_program)(struct cyg_flash_dev* dev, cyg_flashaddr_t dest, const void* src, size_t len)
{
    ...
    am29_dev = (cyg_am29xxxxx_dev*) dev->priv;
    if(am29_dev->wr_buf_sz > 1) {
        int (*program_buffered_fn)(volatile AM29_TYPE*, volatile AM29_TYPE*, const cyg_uint8*, cyg_uint32, int, cyg_uint32);
        program_buffered_fn  = (int (*)(volatile AM29_TYPE*, volatile AM29_TYPE*, const cyg_uint8*, cyg_uint32, int, cyg_uint32))
            cyg_flash_anonymizer( & AM29_FNNAME(am29_hw_program_buffered) );
        AM29_INTSCACHE_BEGIN();
        (*program_buffered_fn)(block, addr, data, len / sizeof(AM29_TYPE), retries, am29_dev->wr_buf_sz);
        AM29_INTSCACHE_END();
