	...
	)
{
	...
	if (   (++ec, ISRESPONSE(inpkt->rm_vn_mode))
        ...
        /* BZ# 9798 
         * Do nothing here. No support to handle private mode (7) packets
         */
#if 0
	...
	)
{
	register struct conf_unpeer *cp;
	register int items;
	register struct peer *peer = NULL;
	struct sockaddr_storage peeraddr;
	int bad;
...
#ifdef NTP_ECOS
        u_char hostname[46];
        if (find_hostname_by_addr(&addr, hostname))
            if (!ISREFCLOCKADR(&tmp_clock) ||
		        findexistingpeer(hostname, (struct peer *)0, -1) == 0) {
#else
		if (!ISREFCLOCKADR(&tmp_clock) ||
