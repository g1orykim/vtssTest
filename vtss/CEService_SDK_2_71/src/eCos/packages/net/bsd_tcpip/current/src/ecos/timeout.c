...
static int vtss_start_ips_tick = 0;
static void
do_alarm(cyg_handle_t alarm, cyg_addrword_t data)
{
    if (vtss_start_ips_tick != 1)
        return;
#else
static void
do_alarm(cyg_handle_t alarm, cyg_addrword_t data)
...
#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
void vtss_start_ips_tick_signal(void)
{
    if (!vtss_start_ips_tick)
        return;

    if (vtss_start_ips_tick == -1) {
        cyg_thread_resume(alarm_thread_handle);    // Start it
        vtss_start_ips_tick = 1;
        cyg_flag_setbits(&alarm_flag, 1);
    }
}
#endif /* VTSS_IPS_IMPL */
...
#if (defined(VTSS_IPS_IMPL) && VTSS_IPS_IMPL)
                /* Make it longer */
                CYG_ASSERT( last_set_time + e->delta + 1973 > now,
