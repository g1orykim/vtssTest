...
/* +++ peter, 2008/1, parse raw text OID subtree string */
static int convert_objid(char *str, oid *oid, size_t *oid_len)
{
    char *value_char;
    int  num = 0, i;
		      ...
		      char *param)
{
    ...
    if (convert_objid(subtree, suboid, &suboid_len)) {
    /* peter, 2008/1, using read_objid() will increase stacksize cause recursive function parse_subtree()
...
vacm_in_view_callback(int majorID, int minorID, void *serverarg,
  ...
  /* peter, 2008/1, don't process it when SNMP engine not supported V3 */
  SnmpdGetVersion(&version);
		  ...
		  size_t namelen)
{
	...
	if (sp == NULL && !vacm_is_configured()) {
        ...
        /* peter, 2007/12, don't allow running without SNMPv3 community entry
		return 0; */
		return 1;
	    default:
		return 1;
    ...
    /* peter, 2008/1, check contextPrefix
    ap = vacm_getAccessEntry(gp->groupName, "", pdu->securityModel,
                             pdu->securityLevel); */
    ap = vacm_getAccessEntry(gp->groupName, pdu->contextName == NULL ? "" : pdu->contextName, pdu->securityModel,
                             pdu->securityLevel);
...
void SnmpdDelCommunityEntry (unsigned char *community_p)
{
    struct vacm_securityEntry *sp = NULL, *pre_sp = NULL, *del_sp = NULL;
    int found_falg = 0;

    sp = pre_sp = securityFirst; 
    while (sp != NULL) {
        if (!strcmp((char *) community_p, sp->community)) {
            pre_sp->next = sp->next;
            del_sp = sp;
            found_falg = 1;
            break;
    ...
    if (found_falg) {
        if (del_sp == securityFirst) {
            securityFirst = del_sp->next;
        }
        if (del_sp == securityLast) {
            securityLast = pre_sp;
        }
        free(del_sp);
    }
