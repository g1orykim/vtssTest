...
// Function for tranfering file data
void 
cyg_httpd_send_content_disposition(cyg_httpd_ires_table_entry *entry)
{
    ...
    if(httpstate.status_code == CYG_HTTPD_STATUS_OK) {
    ...
    snprintf(buffer,sizeof(buffer),"%s\"%s\"; %s%u\r\n",
             ...
             "size=",
             entry->f_size);
    cyg_httpd_header_custom(&httpstate,&buffer[0]);
...
#ifdef CYGOPT_NET_ATHTTPD_HTTPS
    if (ssl)
        SSL_write(ssl, httpstate.outbuffer, header_length);
    else
#endif /* CYGOPT_NET_ATHTTPD_HTTPS */
        send(descr, httpstate.outbuffer, header_length, 0);

    if (httpstate.mode & CYG_HTTPD_MODE_SEND_HEADER_ONLY) 
        return;
    cyg_httpd_write((char *)entry->f_ptr, entry->f_size);
}
...
cyg_httpd_send_ires(cyg_httpd_ires_table_entry *entry)
{
    ...
    if(httpstate.in_etag &&
       strncmp(http_ires_etag, httpstate.in_etag, strlen(http_ires_etag)) == 0) {
        /* Match */
        httpstate.status_code  = CYG_HTTPD_STATUS_NOT_MODIFIED;
        httpstate.mode |= CYG_HTTPD_MODE_SEND_HEADER_ONLY;
    } else {
        /* Use ETag for header when sending internal resource */
        cyg_httpd_header_flag(&httpstate, CYG_HTTPD_HDR_ETAG);
    }
    ...
    if(httpstate.status_code == CYG_HTTPD_STATUS_OK) {
        ...
        /* Zipped ? */
        if(entry->f_ptr[0] == 0x1f && entry->f_ptr[1] == 0x8b) /* ID1 && ID2 */
            cyg_httpd_header_flag(&httpstate, CYG_HTTPD_HDR_ENCODING_GZIP);
    }

#ifdef CYGOPT_NET_ATHTTPD_USE_IRES_HEADER
    cyg_httpd_header_custom(&httpstate, CYGOPT_NET_ATHTTPD_USE_IRES_HEADER);
#endif /*  CYGOPT_NET_ATHTTPD_USE_IRES_HEADER */
