...
usm_parse_create_usmUser(const char *token, char *line) {
  ...
  /* peter, 2007/12, upgrade to new version 1.56.2.8
...
add:
  /* peter, 2007/12, set user storage type as permanent */
  newuser->userStorageType = SNMP_STORAGE_PERMANENT;
...
init_snmpv3(const char *type) {
  ...
  if (type && !strcmp(type,"snmpapp")) {
    ...
    /* peter, 2007/12, move initalization engine ID to vtss_snmp.c
...
{
    u_char *bufp = NULL;
    static int SnmpdSetEngineId_Boots = 1;
    ...
    /* check engine ID change */
    if (engineID && !memcmp(engineID, engine_id, engineIDLength > len ? engineIDLength : len)) {
        return len;
    }
    ...
    memcpy(bufp, engine_id, len);
    SNMP_FREE(engineID);
    engineID = bufp;
    engineIDLength = len;

    /* set our local engineTime in the LCD timing cache */
    snmpv3_set_engineBootsAndTime(SnmpdSetEngineId_Boots++, 0);
