...
snmp_pdu_create(int command)
{
    ...
    /* peter, 2007/9, upgrade to new version 1.70.2.7 */
    pdu->time			 = 0;
...
#if 1 /* peter, 2007/9, upgrade to new version 1.70.2.7 */
...
#endif /* peter, 2007/9, upgrade to new version 1.70.2.7 */
			 ...
			 struct snmp_pdu **response)
{
	...
	if (count > 0){
	    ...
	    case 0:
        /* peter, 2007/12, we need retry here to let response packet can handled correctly
		...
		retry_cnt++;
		cyg_thread_delay(CYG_SNMP_MSEC2TICK(10));
		continue;
	    case -1:
		if (errno == EINTR){
		    continue;
