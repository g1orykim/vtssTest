...
get_netinfo_config()
{
	...
	if (status != NI_OK) {
   	...
   	config = (struct netinfo_config_state *)ntp_callout_malloc(sizeof(struct netinfo_config_state));
	...
	)
{
	...
	if (!val_list) {
			 ...
			 */
				...
				if (! (val_list = config->val_list = (char**)ntp_callout_malloc(sizeof(char*) * (namelist.ni_namelist_len + 1))))
				...
				for (index = 0; index < namelist.ni_namelist_len; index++) {
					...
					if (! (val_list[index] = (char*)ntp_callout_malloc(strlen(value)+1)))
						{ msyslog(LOG_ERR, "out of memory while configuring"); break; }
	...
	for (val_index = 0; val_list[val_index]; val_index++)
		ntp_callout_free(val_list[val_index]);
       	ntp_callout_free(val_list);	val_list = config->val_list = NULL; val_index = config->val_index = 0;
