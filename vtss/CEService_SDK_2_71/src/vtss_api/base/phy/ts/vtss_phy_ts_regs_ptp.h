...
/**
 * \brief
 * Defines how flow matching is performed in each encapsulation engine for Gen-1.
 *
 * \details
 * For each engine
 * 0 = Match any flow
 ...
 * \brief
 * Defines how flow matching is performed in each encapsulation engine for
 * ingress side. Only valid values are: 111 : Strict flow for all engines.
 * 000 : Non-strict flow for all engines.
 *
 * \details
 * 0: Match any flow
...
/**
 * \brief
 * When set, indicates an a frame was processod by rewriter command before
...
/**
 * \brief
 * When set, indicates an a frame was processod by analyzer command before
 ...
 * into LTC timer either by serail or parallel interface.
 *
 * \details
...
/**
 * \brief
 * When enabled the asynchronoius load_save_i input is looped back to the
 * pps_o output pin
 *
 * \details
...
/**
 * \brief
 * Load save input input inversion
 *
 * \details
 * 0: Not inverted
 * 1: Inverted
...
/**
 * \brief
 * This offset is added to/substracted from the LTC value whenever
 * LTC_OFFSET_ADJ transistions from 0->1. The maximum value of the field in
...
/**
 * \brief LTC Frequency Synthesis pusle generation
 *
 * \details
 * LTC Frequency Synthesis pusle generation
 *
...
/**
 * \brief
 * When set indicates the error in the calibration. Either could not
 * calibrate or got a data signal while in the calibration mode.
 *
 * \details
