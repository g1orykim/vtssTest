...
static u32 l26_chip_prio(vtss_state_t *vtss_state, const vtss_prio_t prio)
{
    if (prio >= L26_PRIOS) {
...
vtss_rc vtss_l26_qos_port_conf_set(vtss_state_t *vtss_state, const vtss_port_no_t port_no)
{
    ...
    L26_WR(VTSS_ANA_PORT_QOS_CFG(port),
           ...
           VTSS_F_ANA_PORT_QOS_CFG_QOS_DEFAULT_VAL(l26_chip_prio(vtss_state, conf->default_prio)) |
    ...
    for (pcp = VTSS_PCP_START; pcp < VTSS_PCP_END; pcp++) {
        for (dei = VTSS_DEI_START; dei < VTSS_DEI_END; dei++) {
            L26_WR(VTSS_ANA_PORT_QOS_PCP_DEI_MAP_CFG(port, (8*dei + pcp)),
                   ...
                   VTSS_F_ANA_PORT_QOS_PCP_DEI_MAP_CFG_QOS_PCP_DEI_VAL(l26_chip_prio(vtss_state, conf->qos_class_map[pcp][dei])));
        }
    }
...
static vtss_rc l26_qos_conf_set(vtss_state_t *vtss_state, BOOL changed)
{
    ...
    for (i = 0; i < 64; i++) {
        L26_WR(VTSS_ANA_COMMON_DSCP_CFG(i),
               ...
               VTSS_F_ANA_COMMON_DSCP_CFG_QOS_DSCP_VAL(l26_chip_prio(vtss_state, conf->dscp_qos_class_map[i])) |
