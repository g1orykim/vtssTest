...
#endif
extern char fis_name_of_latest_loaded_image[];
...
flash_fis_op2( int op, unsigned int index, struct fis_table_entry *entry)
{
   ...
   switch ( op ) {
      ...
      case CYGNUM_CALL_IF_FLASH_FIS_MODIFY_ENTRY:
         ...
         break;
      case CYGNUM_CALL_IF_FLASH_FIS_GET_LOADED_ENTRY:
          CYG_ASSERT(entry!=0, "fis_table_entry == 0");
          memcpy(entry->name, fis_name_of_latest_loaded_image, 16);
          res = 1; // Return something different from 0 to be backward compatible, so that the Application code can check for it.
          break;
      default:
         break;
   }
...
#ifdef CYGSEM_HAL_VIRTUAL_VECTOR_PERSIST_DATA_CLAIM
CYG_ADDRESS persist_data_start;
size_t persist_data_length;
size_t get_persist_data(cyg_persist_data_t *p)
{
    p->buffer = persist_data_start;
    return (p->length = persist_data_length);
}
#endif
...
#if 0
        diag_printf("%s: usecs: %d (us/p=%d, t/us=%d)\n", __FUNCTION__, 
                    usecs, usec_per_period, ticks_per_usec);
#endif
...
#if 0
            diag_printf("%s: elapsed: %d, slice %u, elapsed_usec %d\n", __FUNCTION__, 
                        elapsed, slice, elapsed_usec);
#endif
...
#ifdef CYGSEM_HAL_VIRTUAL_VECTOR_PERSIST_DATA_CLAIM
    CYGACC_CALL_IF_GET_PERSIST_DATA_SET(get_persist_data);
#endif
