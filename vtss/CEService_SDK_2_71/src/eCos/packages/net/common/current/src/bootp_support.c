...
/* this variable should be moved to be global variable, because SNTP moduel uses it directly */
/* this issue also exists in original eCos codes */
#ifdef CYGNUM_NET_SNTP_UNICAST_MAXDHCP
/* peter, 2010/11/30, don't use point for 'sntp_servers'
sntp_server_list_t ntp_servers[CYGNUM_NET_SNTP_UNICAST_MAXDHCP]; */
#endif
...
#define MAX_IP_ADDR_LEN 16
        ...
        unsigned int buf_uint[BP_MAX_OPTION_LEN/4+1];
        char *buf;  
        
        buf = (char *)buf_uint;
        memset(buf,0,sizeof(buf_uint));
        length = sizeof(buf_uint);
...
#ifdef CYGNUM_NET_SNTP_UNICAST_MAXDHCP
    {
        sntp_server_list_t  ntp_servers[2];
        sntp_server_list_t  temp_ntp_servers[2];
        struct in_addr      dhcp_addrs[1]; //We only support one entry now
        //struct in_addr      dhcp_addrs[CYGNUM_NET_SNTP_UNICAST_MAXDHCP];
        cyg_uint32          count = 2; //must equal 2, one is for IPv4 and another is for IPv6
        cyg_uint32          i, num_servers = 0;

        /* Removed any previously registered addresses */
        /* peter, 2010/11/30, cannot remove IPv6 SNTP server
        cyg_sntp_set_servers(NULL, 0); */
        memset(&temp_ntp_servers, 0, sizeof(temp_ntp_servers));
        cyg_sntp_get_servers(ntp_servers, &count);
        for (i = 0; i < count; i++) {
            if (ntp_servers[i].ip_sntp_server.sa_family == AF_INET6) {
                memcpy(&temp_ntp_servers[0], &ntp_servers[i], sizeof(sntp_server_list_t));
                num_servers = 1;
                break;
            }
        }
        ...
        if (get_bootp_option(bp, TAG_NTP_SERVER, &dhcp_addrs[0], &length))
        {
            ...
            {
                //struct sockaddr_in *saddr = (struct sockaddr_in *)&ntp_servers[num].ip_sntp_server;
                struct sockaddr_in *saddr = (struct sockaddr_in *)&temp_ntp_servers[num_servers++].ip_sntp_server;
            ...
            cyg_sntp_set_servers(temp_ntp_servers, num_servers);
        }
    }
...
{
#if 0 // Simon, Don't add IPv4 mapped IPv6 Address when getting an IPv4 address from DHCP server
    int s =-1 ;
