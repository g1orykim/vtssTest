 ...
 * should be 10 ms.
 */
#if defined RELIANTUNIX_CLOCK || defined SCO5_CLOCK || defined NTP_ECOS
...
#if 0	    
	    printf("sys_residual %.6f now %.6f isneg %d  adjtv.tv_sec %ld  adjtv.tv_usec %ld\n",sys_residual,now, isneg, adjtv.tv_sec, adjtv.tv_usec);
#endif
#if defined(HAVE_CLOCK_GETTIME) || defined(HAVE_GETCLOCK)
# ifdef HAVE_CLOCK_GETTIME
