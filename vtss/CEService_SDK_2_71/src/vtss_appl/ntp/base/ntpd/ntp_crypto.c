	...
	)
{
	...
	if (cinf->subject != NULL)
		ntp_callout_free(cinf->subject);
	if (cinf->issuer != NULL)
		ntp_callout_free(cinf->issuer);
	if (cinf->grpkey != NULL)
		ntp_callout_free(cinf->grpkey);
	value_free(&cinf->cert);
	ntp_callout_free(cinf);
}
	...
	)
{
	...
	ntp_callout_free(header);
	if (strcmp(name, "CERTIFICATE") !=0) {
		msyslog(LOG_INFO, "crypto_cert: wrong PEM type %s",
		    name);
		ntp_callout_free(name);
		ntp_callout_free(data);
		(void)fclose(str);
		return (NULL);
	}
	ntp_callout_free(name);
