...
dump_frame(unsigned char *frame)
{
    ...
    for (i = HAL_THREAD_CONTEXT_FIRST;  i <= HAL_THREAD_CONTEXT_LAST;  i++) {
        if ((i - HAL_THREAD_CONTEXT_FIRST) % 6 == 0) diag_printf("\nR%d: ", i);
        diag_printf("%08X ", rp->d[i-HAL_THREAD_CONTEXT_FIRST]);
    }
    diag_printf("\n");
    diag_printf("FP: %08X, IP: %08X, SP: %08X\n",
                rp->fp, rp->ip, rp->sp);
    diag_printf("LR: %08X, PC: %08X, PSR: %08X\n",
                rp->lr, rp->pc, rp->cpsr);
}
#endif
